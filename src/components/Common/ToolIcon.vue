...
const props = withDefaults(defineProps<{
  /** 图标：图片路径（以 / 或 http 开头）或 @nicons/fa 图标名（如 "ScrewdriverWrench"） */
  logo: string
  /** 图标尺寸（px），默认 36 */
  size?: number
...
/** 同步获取 @nicons/fa 图标组件，避免 defineAsyncComponent 导致的闪烁 */
const iconComponent = computed<Component | null>(() => {
  if (!isIconName.value) return null
  const comp = iconMap[props.logo]
  if (!comp) {
    console.warn(`[ToolIcon] @nicons/fa 中未找到图标: "${props.logo}"`)
    return null
  }
  return comp
})
...
<template>
  ...
  >
    <template #error>
      <div
        :class="['bg-gradient-to-br from-primary-100 to-primary-200 dark:from-slate-700 dark:to-slate-600 flex items-center justify-center', rounded]"
        :style="{ width: size + 'px', height: size + 'px' }"
      >
        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-primary-400 dark:text-primary-500" viewBox="0 0 512 512" fill="currentColor">
