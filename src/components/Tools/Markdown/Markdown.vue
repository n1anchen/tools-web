    ...
    <ToolHero summary="从一个想法，到一份可以交付的文档">
      <template #metrics>
        <div class="hero-metrics">
          <div><span>有效字符</span><strong>{{ statistics.charactersWithoutWhitespace }}</strong></div>
          <div><span>标题层级</span><strong>{{ headingCount }}</strong></div>
          <div><span>分钟阅读</span><strong>{{ statistics.readingMinutes || '—' }}</strong></div>
        </div>
      </template>
    </ToolHero>
