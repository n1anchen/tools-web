    ...
    <ToolHero summary="找到表情，收藏起来，点一下就复制">
      <template #metrics>
        <div class="hero-metrics">
          <div><span>收录表情</span><strong>{{ emojiData.length }}</strong></div>
          <div><span>我的收藏</span><strong>{{ favorites.length }}</strong></div>
          <div><span>最近使用</span><strong>{{ recent.length }}</strong></div>
        </div>
      </template>
    </ToolHero>
