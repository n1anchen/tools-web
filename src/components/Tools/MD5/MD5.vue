...
import { Check, CopyDocument, Delete, Document, UploadFilled } from '@element-plus/icons-vue'
...
<template>
  <div class="md5-page flex flex-col mt-3 flex-1">
    <ToolHero summary="计算摘要，不再误叫“加密”" description="适合文件校验、缓存键等兼容场景；MD5 不适合密码存储或安全签名。" />
...
<style scoped>
.md5-page { --blue: #2563eb; gap: 16px; }.panel, .knowledge-card { border: 1px solid var(--c-border); border-radius: var(--radius-card); background: var(--c-surface); box-shadow: var(--shadow-card); }.eyebrow { color: var(--blue); font-size:12px; font-weight: 800; letter-spacing: .15em; }
.mode-tabs { display: grid; grid-template-columns: repeat(2, 1fr); gap: 9px; }.mode-tabs button { display: flex; align-items: center; gap: 11px; padding: 13px 16px; border: 1px solid var(--c-border); border-radius: var(--radius-lg); color: var(--c-text-secondary); background: var(--c-surface); text-align: left; cursor: pointer; transition: .18s ease; }.mode-tabs button:hover, .mode-tabs button.active { border-color: #93c5fd; background: #eff6ff; transform: translateY(-2px); }.mode-tabs button > span { display: grid; width: 34px; height: 34px; flex: none; place-items: center; border-radius: var(--radius-sm); color: var(--c-on-accent); background: #94a3b8; font: 800 12px ui-monospace, monospace; }.mode-tabs button.active > span { background: var(--blue); }.mode-tabs div { display: flex; flex-direction: column; }.mode-tabs strong { color: var(--c-text-strong); font-size: 12px; }.mode-tabs small { color: var(--c-text-muted); font-size:12px; }
.workspace { display: grid; grid-template-columns: minmax(0, 1.2fr) minmax(330px, .8fr); align-items: start; gap: 16px; }.input-column { display: flex; min-width: 0; flex-direction: column; gap: 16px; }.panel { padding: 21px; }.panel-heading { display: flex; align-items: center; justify-content: space-between; gap: 12px; margin-bottom: 15px; }.panel-heading h3 { margin: 3px 0 0; color: var(--c-text-primary); font-size: 16px; }.input-stats { display: flex; gap: 6px; }.input-stats span, .privacy-badge { padding: 4px 7px; border-radius: 99px; color: var(--c-text-secondary); background: #f1f5f9; font-size:12px; }.input-panel :deep(.el-textarea__inner), .compare-panel :deep(.el-input__wrapper) { border-radius: var(--radius-md); box-shadow: 0 0 0 1px #e2e8f0 inset; font-family: ui-monospace, monospace; }.example-row { display: flex; flex-wrap: wrap; align-items: center; gap: 6px; margin-top: 11px; }.example-row > span { color: var(--c-text-muted); font-size:12px; }.example-row button { padding: 4px 8px; border: 0; border-radius: 99px; color: var(--c-text-body); background: #f1f5f9; font-size:12px; cursor: pointer; }.drop-zone { display: flex; min-height: 250px; align-items: center; justify-content: center; flex-direction: column; border: 1px dashed #bfdbfe; border-radius: var(--radius-lg); background: var(--c-surface-subtle); transition: .18s ease; }.drop-zone.dragging { border-color: #2563eb; background: #eff6ff; }.drop-zone > .el-icon { color: #3b82f6; font-size: 36px; }.drop-zone strong { margin-top: 10px; color: var(--c-text-strong); }.drop-zone span { margin: 3px 0 15px; color: var(--c-text-muted); font-size:12px; }.drop-zone label, .replace-file { padding: 7px 13px; border-radius: var(--radius-sm); color: var(--c-on-accent); background: #2563eb; font-size:12px; cursor: pointer; }.drop-zone input, .replace-file input { display: none; }.file-progress-card { padding: 17px; border-radius: var(--radius-lg); background: var(--c-surface-subtle); }.file-main { display: flex; align-items: center; gap: 11px; margin-bottom: 17px; }.file-icon { display: grid; width: 43px; height: 43px; flex: none; place-items: center; border-radius: var(--radius-md); color: #2563eb; background: #dbeafe; }.file-main > div:nth-child(2) { display: flex; min-width: 0; flex: 1; flex-direction: column; }.file-main strong { color: var(--c-text-strong); overflow: hidden; text-overflow: ellipsis; font-size:12px; white-space: nowrap; }.file-main span { color: var(--c-text-muted); font-size:12px; }.replace-file { display: block; width: fit-content; margin-top: 14px; }.compare-status { color: var(--c-text-muted); font-size:12px; }.compare-status.match { display: flex; align-items: center; gap: 4px; color: #16a34a; }.compare-status.mismatch, .compare-status.invalid { color: #dc2626; }.compare-panel > p { margin: 9px 0 0; color: var(--c-text-muted); font-size:12px; }
.result-panel { position: sticky; top: 14px; }.digest-visual { padding: 15px; border-radius: var(--radius-md); color: #dbeafe; background: linear-gradient(145deg, #172554, #1e3a8a); }.digest-visual > span { font-size:12px; letter-spacing: .15em; }.digest-visual code { display: block; margin: 8px 0 13px; overflow-wrap: anywhere; color: var(--c-on-accent); font-size: 13px; }.digest-visual > div { display: grid; grid-template-columns: repeat(16, 1fr); gap: 3px; }.digest-visual i { height: 21px; border-radius: 3px; background: #60a5fa; }.result-list { display: flex; flex-direction: column; gap: 7px; margin-top: 12px; }.result-list button { display: grid; grid-template-columns: 84px minmax(0, 1fr) auto; align-items: center; gap: 8px; padding: 10px; border: 1px solid var(--c-border); border-radius: var(--radius-sm); color: var(--c-text-secondary); background: var(--c-surface-subtle); text-align: left; cursor: pointer; }.result-list button.primary { border-color: #bfdbfe; background: #eff6ff; }.result-list span { font-size:12px; }.result-list code { overflow: hidden; color: var(--c-text-strong); text-overflow: ellipsis; font-size:12px; white-space: nowrap; }.result-empty { display: grid; min-height: 250px; place-items: center; color: var(--c-text-muted); font-size:12px; }.result-meta { display: grid; grid-template-columns: repeat(3, 1fr); gap: 6px; margin-top: 13px; }.result-meta div { display: flex; flex-direction: column; padding: 8px; border-radius: var(--radius-sm); background: #f1f5f9; }.result-meta span { color: var(--c-text-muted); font-size:12px; }.result-meta strong { color: var(--c-text-body); font-size:12px; }
.knowledge-card { padding: 21px; }.knowledge-card h3 { margin: 3px 0 0; color: var(--c-text-primary); font-size: 17px; }.knowledge-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 9px; margin-top: 15px; }.knowledge-grid article { padding: 13px; border-radius: var(--radius-md); background: var(--c-surface-subtle); }.knowledge-grid strong { color: var(--c-text-strong); font-size:12px; }.knowledge-grid p { margin: 5px 0 0; color: var(--c-text-secondary); font-size:12px; line-height: 1.65; }
:global(html.dark .md5-page .panel), :global(html.dark .md5-page .knowledge-card), :global(html.dark .md5-page .mode-tabs button) { border-color: var(--c-border); background: var(--c-surface); box-shadow: none; }:global(html.dark .md5-page h3), :global(html.dark .md5-page .mode-tabs strong), :global(html.dark .md5-page .file-main strong), :global(html.dark .md5-page .drop-zone strong), :global(html.dark .md5-page .result-list code), :global(html.dark .md5-page .knowledge-grid strong) { color: #f8fafc; }:global(html.dark .md5-page .mode-tabs button.active), :global(html.dark .md5-page .result-list button.primary) { border-color: #1d4ed8; background: #172554; }:global(html.dark .md5-page .drop-zone), :global(html.dark .md5-page .file-progress-card), :global(html.dark .md5-page .result-list button), :global(html.dark .md5-page .result-meta div), :global(html.dark .md5-page .knowledge-grid article) { border-color: var(--c-border); color: var(--c-text-secondary); background: var(--c-surface-subtle); }
@media (max-width: 920px) { .workspace { grid-template-columns: 1fr; }.result-panel { position: static; } }
@media (max-width: 640px) { .mode-tabs { grid-template-columns: 1fr; }.panel, .knowledge-card { padding: 17px; border-radius: 19px; }.result-list button { grid-template-columns: 78px minmax(0, 1fr) auto; }.knowledge-grid { grid-template-columns: 1fr; }.result-meta { grid-template-columns: 1fr 1fr; } }
</style>
