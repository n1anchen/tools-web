...
.control-note { flex: 1; color: var(--c-text-muted); font-size: 11px; line-height: 1.6; }
...
.total-block { min-width: 170px; padding-right: 24px; border-right: 1px solid var(--c-border); }
...
:global(html.dark .dice-page .control-row label > span), :global(html.dark .dice-page .die-shell > strong), :global(html.dark .dice-page .result-metrics b), :global(html.dark .dice-page .history-list strong), :global(html.dark .dice-page .guide-grid strong) { color: var(--c-text-primary); }
:global(html.dark .dice-page .control-row), :global(html.dark .dice-page .result-metrics div), :global(html.dark .dice-page .history-list button), :global(html.dark .dice-page .guide-grid div) { background: var(--c-surface-subtle); }
:global(html.dark .dice-page .die-shell) { border-color: var(--c-border); background: rgba(15, 23, 42, .8); }
:global(html.dark .dice-page .total-block), :global(html.dark .dice-page .history-section) { border-color: var(--c-border); }@media (max-width: 900px) { .dice-grid { grid-template-columns: repeat(3, 135px); } .result-summary { align-items: stretch; flex-direction: column; } .total-block { border-right: 0; border-bottom: 1px solid var(--c-border); padding: 0 0 18px; } .history-list { grid-template-columns: repeat(2, 1fr); }}@media (max-width: 640px) { .workspace-card, .dice-board { padding: 18px; } .preset-row { flex-wrap: wrap; } .control-row { align-items: stretch; flex-direction: column; } .control-row label { width: 100%; } .dice-grid, .dice-grid.count-2 { grid-template-columns: repeat(2, 130px); } .dice-grid.count-1 { grid-template-columns: 150px; } .board-actions { width: 100%; flex-direction: column; gap: 8px; } .board-actions .el-button { width: 100%; margin: 0; } .result-metrics { grid-template-columns: repeat(2, 1fr); } .history-list, .guide-grid { grid-template-columns: 1fr; }}</style>
