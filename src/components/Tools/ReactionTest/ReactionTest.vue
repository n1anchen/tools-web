    ...
    <ToolHero summary="不是点一次，而是一场完整测试">
      <template #metrics>
        <div class="hero-metrics">
          <div><span>个人最佳</span><strong>{{ personalBest ? `${personalBest} ms` : '等待记录' }}</strong></div>
        </div>
      </template>
    </ToolHero>
...
<style scoped>
.reaction-page { --blue: #2563eb; gap: 16px; }.session-bar, .stats-card, .history-card, .tips-grid article { border: 1px solid #e2e8f0; border-radius: 22px; background: #fff; box-shadow: 0 12px 35px rgb(15 23 42 / 6%); }.eyebrow { color: var(--blue); font-size:12px; font-weight: 800; letter-spacing: .15em; }.hero-metrics{display:grid;grid-template-columns:minmax(160px,1fr);min-width:200px;overflow:hidden;border:1px solid #e0e9f4;border-radius:18px;background:rgba(255,255,255,.78)}.hero-metrics div{padding:12px 14px;text-align:center;border-left:1px solid #e5edf6}.hero-metrics div:first-child{border-left:0}.hero-metrics span,.hero-metrics strong{display:block}.hero-metrics span{margin-top:4px;color:#7a899c;font-size:12px}.hero-metrics strong{overflow:hidden;color:#334155;font-size:18px;text-overflow:ellipsis;white-space:nowrap}
...
:global(html.dark .reaction-page .session-bar), :global(html.dark .reaction-page .stats-card), :global(html.dark .reaction-page .history-card), :global(html.dark .reaction-page .tips-grid article) { border-color: #334155; background: #1e293b; box-shadow: none; }:global(html.dark .reaction-page .hero-metrics){border-color:#40516a;background:rgba(15,23,42,.5)}:global(html.dark .reaction-page .hero-metrics div){border-color:#40516a}:global(html.dark .reaction-page .hero-metrics strong){color:#e7edf6}:global(html.dark .reaction-page .hero-metrics span){color:#a8b4c5}:global(html.dark .reaction-page h2), :global(html.dark .reaction-page h3), :global(html.dark .reaction-page .stat-grid strong), :global(html.dark .reaction-page .tips-grid strong) { color: #f8fafc; }:global(html.dark .reaction-page .round-options button), :global(html.dark .reaction-page .stat-grid > div), :global(html.dark .reaction-page .history-chart > div > div) { color: #cbd5e1; background: #0f172a; }:global(html.dark .reaction-page .round-options button.active) { color: #fff; background: #2563eb; }
...
@media (max-width: 640px) { .hero-metrics{grid-template-columns:1fr}.hero-metrics div{border-left:0;border-bottom:1px solid #e5edf6}.hero-metrics div:last-child{border-bottom:0}.session-bar { align-items: flex-start; flex-direction: column; }.session-progress { width: 100%; grid-template-columns: auto minmax(0, 1fr) auto; }.reaction-stage { min-height: 360px; }.stats-card, .history-card { padding: 17px; border-radius: 19px; }.tips-grid { grid-template-columns: 1fr; } }
</style>
