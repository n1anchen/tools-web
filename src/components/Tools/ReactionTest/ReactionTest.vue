...
<template>
  <div class="reaction-page flex flex-col mt-3 flex-1">
    <ToolHero summary="不是点一次，而是一场完整测试">
      <template #metrics>
        <MetricsBar :items="[{ label: '个人最佳', value: personalBest ? `${personalBest} ms` : '等待记录' }]" />
      </template>
    </ToolHero>
...
<style scoped>
.reaction-page { --blue: #2563eb; gap: 16px; }.session-bar, .stats-card, .history-card, .tips-grid article { border: 1px solid var(--c-border); border-radius: var(--radius-card); background: var(--c-surface); box-shadow: var(--shadow-card); }.eyebrow { color: var(--blue); font-size:12px; font-weight: 800; letter-spacing: .15em; }
.session-bar { display: flex; align-items: center; justify-content: space-between; gap: 18px; padding: 13px 17px; }.round-options { display: flex; align-items: center; gap: 6px; }.round-options > span { margin-right: 4px; color: var(--c-text-secondary); font-size:12px; }.round-options button { padding: 6px 10px; border: 0; border-radius: var(--radius-sm); color: var(--c-text-secondary); background: #f1f5f9; font-size:12px; cursor: pointer; }.round-options button.active { color: var(--c-on-accent); background: #2563eb; }.round-options button:disabled { cursor: not-allowed; opacity: .5; }.session-progress { display: grid; grid-template-columns: auto 150px auto; align-items: center; gap: 8px; color: var(--c-text-secondary); font-size:12px; }.session-progress > div { height: 6px; overflow: hidden; border-radius: 99px; background: #e2e8f0; }.session-progress i { display: block; height: 100%; border-radius: inherit; background: linear-gradient(90deg, #3b82f6, #22c55e); transition: width .3s ease; }.session-progress small { color: #ef4444; }
.challenge-grid { display: grid; grid-template-columns: minmax(0, 1.5fr) minmax(280px, .65fr); align-items: stretch; gap: 16px; }.reaction-stage { position: relative; display: flex; min-height: 450px; align-items: center; justify-content: center; overflow: hidden; border: 0; border-radius: 24px; color: var(--c-on-accent); background: linear-gradient(145deg, #1d4ed8, #3b82f6); box-shadow: 0 18px 45px rgb(37 99 235 / 23%); outline: none; cursor: pointer; transition: background .16s ease, transform .16s ease; user-select: none; -webkit-tap-highlight-color: transparent; }.reaction-stage:focus-visible { box-shadow: 0 0 0 4px #bfdbfe, 0 18px 45px rgb(37 99 235 / 23%); }.reaction-stage:active { transform: scale(.995); }.state-waiting { color: #422006; background: linear-gradient(145deg, #facc15, #f59e0b); box-shadow: 0 18px 45px rgb(245 158 11 / 24%); }.state-active { background: linear-gradient(145deg, #16a34a, #22c55e); box-shadow: 0 18px 45px rgb(34 197 94 / 25%); }.state-tooEarly { background: linear-gradient(145deg, #dc2626, #f43f5e); box-shadow: 0 18px 45px rgb(244 63 94 / 24%); }.state-roundResult { background: linear-gradient(145deg, #6d28d9, #8b5cf6); }.state-complete { background: linear-gradient(145deg, #0f766e, #14b8a6); }.stage-noise { position: absolute; inset: 0; opacity: .14; background-image: radial-gradient(circle at 20% 25%, #fff 0 1px, transparent 1.5px), radial-gradient(circle at 75% 70%, #fff 0 1px, transparent 1.5px); background-size: 31px 31px, 43px 43px; }.target-rings { position: absolute; display: grid; width: 270px; height: 270px; place-items: center; opacity: .16; }.target-rings i { position: absolute; inset: 0; border: 2px solid currentColor; border-radius: 50%; }.target-rings i:nth-child(2) { inset: 42px; }.target-rings i:nth-child(3) { inset: 84px; }.target-rings > .el-icon { font-size: 32px; }.state-active .target-rings { animation: pulse 1s ease infinite; opacity: .28; }.stage-content { position: relative; display: flex; z-index: 1; align-items: center; flex-direction: column; padding: 30px; text-align: center; }.stage-content > span { font-size:12px; font-weight: 900; letter-spacing: .2em; opacity: .8; }.stage-content > strong { margin-top: 7px; font-size: clamp(34px, 6vw, 57px); line-height: 1.1; }.stage-content p { margin: 10px 0 24px; font-size: 13px; opacity: .85; }.stage-content small { padding: 6px 10px; border: 1px solid rgb(255 255 255 / 28%); border-radius: 99px; font-size:12px; opacity: .8; }
.stats-card, .history-card { padding: 21px; }.section-heading { display: flex; align-items: center; justify-content: space-between; gap: 12px; }.section-heading h3 { margin: 3px 0 0; color: var(--c-text-primary); font-size: 16px; }.section-heading > span { color: var(--c-text-muted); font-size:12px; }.stat-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-top: 17px; }.stat-grid > div { display: flex; min-height: 85px; justify-content: center; flex-direction: column; padding: 12px; border-radius: var(--radius-md); background: var(--c-surface-subtle); }.stat-grid span { color: var(--c-text-muted); font-size:12px; }.stat-grid strong { margin-top: 3px; color: var(--c-text-strong); font-size: 22px; }.stat-grid small { margin-left: 3px; color: var(--c-text-muted); font-size:12px; }.empty-stats { display: flex; min-height: 290px; align-items: center; justify-content: center; flex-direction: column; color: var(--c-text-muted); text-align: center; }.empty-stats > .el-icon { font-size: 35px; }.empty-stats strong { margin-top: 9px; color: var(--c-text-secondary); }.empty-stats span { max-width: 180px; margin-top: 3px; font-size:12px; }.rating-card { display: flex; flex-direction: column; margin-top: 8px; padding: 13px; border-radius: var(--radius-md); color: var(--c-on-accent); background: linear-gradient(135deg, #2563eb, #7c3aed); }.rating-card span, .rating-card small { font-size:12px; opacity: .8; }.rating-card strong { font-size: 17px; }
.history-chart { display: flex; flex-direction: column; gap: 8px; margin-top: 17px; }.history-chart > div { display: grid; grid-template-columns: 28px minmax(0, 1fr) 55px; align-items: center; gap: 8px; }.history-chart span { color: var(--c-text-muted); font: 12px ui-monospace, monospace; }.history-chart > div > div { height: 20px; overflow: hidden; border-radius: 6px; background: #f1f5f9; }.history-chart i { display: block; height: 100%; border-radius: inherit; background: linear-gradient(90deg, #93c5fd, #3b82f6); transition: width .25s ease; }.history-chart i.best { background: linear-gradient(90deg, #86efac, #22c55e); }.history-chart strong { color: var(--c-text-body); font: 12px ui-monospace, monospace; text-align: right; }.tips-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 9px; }.tips-grid article { display: flex; gap: 10px; padding: 14px; border-radius: var(--radius-md); }.tips-grid article > span { display: grid; width: 31px; height: 31px; flex: none; place-items: center; border-radius: var(--radius-sm); color: #2563eb; background: #dbeafe; font: 800 12px ui-monospace, monospace; }.tips-grid strong { color: var(--c-text-strong); font-size:12px; }.tips-grid p { margin: 3px 0 0; color: var(--c-text-secondary); font-size:12px; line-height: 1.55; }
:global(html.dark .reaction-page .session-bar), :global(html.dark .reaction-page .stats-card), :global(html.dark .reaction-page .history-card), :global(html.dark .reaction-page .tips-grid article) { border-color: var(--c-border); background: var(--c-surface); box-shadow: none; }:global(html.dark .reaction-page h2), :global(html.dark .reaction-page h3), :global(html.dark .reaction-page .stat-grid strong), :global(html.dark .reaction-page .tips-grid strong) { color: #f8fafc; }:global(html.dark .reaction-page .round-options button), :global(html.dark .reaction-page .stat-grid > div), :global(html.dark .reaction-page .history-chart > div > div) { color: var(--c-text-secondary); background: var(--c-surface-subtle); }:global(html.dark .reaction-page .round-options button.active) { color: var(--c-on-accent); background: #2563eb; }
@keyframes pulse { 50% { transform: scale(1.08); opacity: .45; } }@media (max-width: 900px) { .challenge-grid { grid-template-columns: 1fr; }.reaction-stage { min-height: 390px; }}@media (max-width: 640px) {.session-bar { align-items: flex-start; flex-direction: column; }.session-progress { width: 100%; grid-template-columns: auto minmax(0, 1fr) auto; }.reaction-stage { min-height: 360px; }.stats-card, .history-card { padding: 17px; border-radius: 19px; }.tips-grid { grid-template-columns: 1fr; }}</style>
