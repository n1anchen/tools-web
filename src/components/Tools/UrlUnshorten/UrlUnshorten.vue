...
.hero-service-state { display:inline-flex; align-items:center; gap:7px; padding:7px 11px; border:1px solid #e0d8c7; border-radius:var(--radius-sm); background:rgba(255,252,245,.8); font-size:12px; }.hero-service-state .state-dot { width:8px; height:8px; flex:none; border-radius:50%; background:#d99b35; }.hero-service-state.online { border-color:#bee2d5; background:rgba(242,252,248,.8); }.hero-service-state.online .state-dot { background:#19a36f; }.hero-service-state strong { color: var(--c-text-strong); font-size:12px; }.hero-service-state small { color: var(--c-text-muted); font-size:11px; }
...
.input-card,.settings-card,.result-card,.safety-card { margin-top:14px; padding:21px; border:1px solid #dfe7e9; border-radius:19px; background:var(--c-surface); box-shadow:0 10px 26px rgba(31,57,61,.045); }.input-heading,.result-heading,.chain-heading { display:flex; align-items:center; justify-content:space-between; gap:12px; }.input-heading h3,.setting-heading h3,.result-heading h3 { margin:4px 0 0; color:var(--ink); font-size:18px; font-weight:800; }.text-button,.chain-heading button { border:0; background:transparent; color:#197a70; font-size:13px; font-weight:700; cursor:pointer; }.url-input-shell { display:flex; align-items:center; gap:8px; margin-top:15px; padding:6px; border:1px solid #cfdcdf; border-radius:var(--radius-md); background:#fafcfc; }.url-input-shell:focus-within { border-color:#69afa8; box-shadow:0 0 0 3px rgba(15,118,110,.08); }.url-input-shell.invalid { border-color:#e1797e; }.lock-mark { display:grid; place-items:center; width:40px; height:40px; border-radius:var(--radius-sm); background:#e8f5f2; color:#197a70; font-size:18px; }.url-input-shell input { flex:1; min-width:0; border:0; outline:0; background:transparent; color:#293543; font:500 14px/1.4 ui-monospace,SFMono-Regular,Menlo,monospace; }.paste-button { border:0; border-radius:var(--radius-sm); padding:10px 13px; background:#edf3f3; color:#4e6d6a; font-size:13px; font-weight:700; cursor:pointer; }.field-error { margin-top:8px; color:#bd3d46; font-size:13px; font-weight:650; }.diagnostics-row,.result-summary { display:grid; grid-template-columns:repeat(4,1fr); gap:0; margin-top:14px; overflow:hidden; border:1px solid #e2e9eb; border-radius:var(--radius-md); }.diagnostics-row div,.result-summary div { min-width:0; padding:11px 14px; border-right:1px solid #e2e9eb; }.diagnostics-row div:last-child,.result-summary div:last-child { border-right:0; }.diagnostics-row span,.result-summary span { display:block; color:#7c8996; font-size:13px; }.diagnostics-row strong,.result-summary strong { display:block; margin-top:4px; overflow:hidden; color:#415162; font-size:14px; white-space:nowrap; text-overflow:ellipsis; }
.settings-card { display:grid; grid-template-columns:minmax(0,1.4fr) minmax(280px,.6fr); gap:20px; }.cleanup-options { display:grid; grid-template-columns:repeat(3,1fr); gap:8px; margin-top:15px; }.cleanup-options button { padding:13px; border:1px solid #dfe6e8; border-radius:var(--radius-md); background:#fafcfc; text-align:left; cursor:pointer; }.cleanup-options button.active { border-color:#5da49c; background:#f0faf7; box-shadow:0 0 0 2px rgba(15,118,110,.07); }.cleanup-options strong,.cleanup-options span { display:block; }.cleanup-options strong { color:#3f4e5d; font-size:14px; }.cleanup-options span { margin-top:4px; color:#81909d; font-size:13px; line-height:1.45; }.cleanup-options button.active strong { color:#126f67; }.tracking-settings { padding:15px; border:1px solid #dfe7e9; border-radius:var(--radius-md); background:#f8fbfb; }.tracking-settings.disabled { opacity:.68; }.tracking-title { display:flex; align-items:center; justify-content:space-between; gap:10px; }.tracking-title strong,.tracking-title span { display:block; }.tracking-title strong { color:#42515e; font-size:14px; }.tracking-title span { margin-top:3px; color:#82909c; font-size:13px; line-height:1.4; }.tracking-settings label { display:grid; grid-template-columns:auto auto; gap:7px 10px; margin-top:17px; color:#687683; font-size:13px; }.tracking-settings label strong { color:#226f69; }.tracking-settings input { grid-column:1/-1; width:100%; accent-color:#0f766e; }.action-row { grid-column:1/-1; display:flex; justify-content:flex-end; gap:9px; padding-top:16px; border-top:1px solid #e8edef; }.primary-button,.secondary-button { display:inline-flex; align-items:center; justify-content:center; gap:8px; min-height:42px; border-radius:var(--radius-sm); padding:10px 16px; font-size:14px; font-weight:750; cursor:pointer; }.primary-button { border:0; background:var(--accent); color:white; }.secondary-button { border:1px solid #cddcde; background:white; color:#356c68; }.primary-button:disabled,.secondary-button:disabled { opacity:.42; cursor:not-allowed; }.spinner { width:13px; height:13px; border:2px solid rgba(255,255,255,.4); border-top-color:#fff; border-radius:50%; animation:spin .8s linear infinite; }@keyframes spin{to{transform:rotate(360deg)}}.service-note { grid-column:1/-1; margin:-8px 0 0; color:#8a7752; font-size:13px; text-align:right; }
.result-badge { padding:7px 10px; border-radius:var(--radius-sm); background:#eaf6f3; color:#18756d!important; font-size:13px!important; letter-spacing:0!important; }.final-url-box { display:grid; grid-template-columns:auto 1fr auto; align-items:center; gap:12px; margin-top:14px; padding:15px; border:1px solid #cfe3de; border-radius:var(--radius-md); background:#f5fbf9; }.url-status-icon { display:grid; place-items:center; width:38px; height:38px; border-radius:50%; background:#dff4ed; color:#168365; font-weight:900; }.final-url-box>div:nth-child(2) { min-width:0; }.final-url-box span,.final-url-box a { display:block; }.final-url-box span { color:#7c8a95; font-size:13px; }.final-url-box a { margin-top:5px; overflow-wrap:anywhere; color:#176f8a; font:650 14px/1.55 ui-monospace,SFMono-Regular,Menlo,monospace; }.url-actions { display:flex; gap:6px; }.url-actions button,.url-actions>a { margin:0; border:1px solid #bdd8d3; border-radius:var(--radius-xs); padding:7px 10px; background:var(--c-surface); color:#176f68; font:700 13px/1.2 sans-serif; text-decoration:none; cursor:pointer; }.removed-list { display:flex; align-items:center; flex-wrap:wrap; gap:7px; margin-top:12px; }.removed-list>span { color:#778590; font-size:13px; font-weight:700; }.removed-list code { padding:5px 8px; border-radius:var(--radius-xs); background:#fff1e8; color:#ad5c2f; font-size:13px; }.chain-section { margin-top:19px; padding-top:18px; border-top:1px solid #e5ebec; }.chain-heading strong { color:#384957; font-size:15px; }.chain-section ol { margin:14px 0 0; padding:0; list-style:none; }.chain-section li { position:relative; display:grid; grid-template-columns:auto 1fr; gap:12px; padding-bottom:16px; }.chain-section li:not(:last-child)::after { content:""; position:absolute; left:15px; top:31px; bottom:2px; width:1px; background:#cadbd8; }.chain-section li>span { display:grid; place-items:center; align-self:start; width:31px; height:31px; border-radius:50%; background:#e7f4f1; color:#197a70; font-size:13px; font-weight:800; }.chain-section li strong,.chain-section li code { display:block; }.chain-section li strong { color:#566674; font-size:13px; }.chain-section li code { margin-top:4px; overflow-wrap:anywhere; color:#6f7d89; font-size:13px; line-height:1.5; }
.safety-card { display:flex; align-items:center; justify-content:space-between; gap:16px; padding:15px 18px; }.safety-card>div:first-child { display:flex; align-items:center; gap:11px; }.safety-card>div:first-child>span { display:grid; place-items:center; width:38px; height:38px; flex:none; border-radius:var(--radius-sm); background:#eef5ff; color:#4b73a4; font-size:13px; font-weight:800; }.safety-card p { margin:0; color:#73808d; font-size:13px; line-height:1.55; }.safety-card p strong { display:block; color:#455462; font-size:14px; }.example-buttons { display:flex; gap:7px; flex:none; }.example-buttons button { border:1px solid #dbe4e6; border-radius:var(--radius-sm); padding:8px 10px; background:var(--c-surface); color:#52706e; font-size:13px; cursor:pointer; }
