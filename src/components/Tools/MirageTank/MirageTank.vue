...
<template>
  <div class="mirage-tool flex flex-col mt-3 flex-1">
    <ToolHero summary="制作与显形，在同一个工作台完成">
      <template #metrics>
        <MetricsBar :items="[{ label: '当前模式', value: activeMode === 'encode' ? '制作坦克图' : '显形解码' }, { label: '处理状态', value: activeStatus }, { label: '输出尺寸', value: activeMode === 'encode' ? encodeDimensions : decodeDimensions }]" />
      </template>
    </ToolHero>
...
<style scoped>
.mirage-tool { --accent:#2d6eb5; --ink:#262d38; --muted:#707b89; }.step-heading>div>span,.settings-title>div>span { color:#557b9f; font-size:13px; font-weight:800; letter-spacing:.12em; }
.mode-tabs { display:grid; grid-template-columns:repeat(2,1fr); gap:10px; margin-top:14px; }.mode-tabs button { display:flex; align-items:center; gap:12px; padding:14px; border:1px solid #dfe4ea; border-radius:var(--radius-md); background:var(--c-surface); text-align:left; cursor:pointer; }.mode-tabs button>span { display:grid; place-items:center; width:41px; height:41px; border-radius:var(--radius-sm); background:#edf3f8; color:#507ba1; font-size:16px; font-weight:800; }.mode-tabs strong,.mode-tabs small { display:block; }.mode-tabs strong { color:#3a4653; font-size:14px; }.mode-tabs small { margin-top:3px; color:#87909a; font-size:13px; }.mode-tabs button.active { border-color:#77a4cd; background:#f8fbff; box-shadow:0 0 0 3px rgba(45,110,181,.07); }.mode-tabs button.active>span { background:#e5f0fb; color:#2467a7; }
.workbench-card { margin-top:14px; padding:21px; border:1px solid #dfe4ea; border-radius:var(--radius-card); background:var(--c-surface); box-shadow:0 11px 28px rgba(40,54,69,.045); }.step-heading,.settings-title { display:flex; align-items:center; justify-content:space-between; gap:12px; }.step-heading h3,.settings-title h3 { margin:4px 0 0; color:var(--ink); font-size:18px; font-weight:800; }.step-actions { display:flex; gap:6px; }.step-actions button,.reset-button,.settings-title button { border:1px solid #d7dfe7; border-radius:var(--radius-xs); padding:7px 10px; background:var(--c-surface); color:#52708c; font-size:13px; font-weight:700; cursor:pointer; }.step-actions button:disabled,.settings-title button:disabled { opacity:.4; cursor:not-allowed; }.source-grid { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:12px; margin-top:15px; }.source-card { min-width:0; padding:15px; border:1px solid #e1e5e9; border-radius:var(--radius-md); background:#fbfcfd; }.source-title { display:flex; align-items:center; gap:10px; }.source-number { display:grid; place-items:center; width:34px; height:34px; border-radius:var(--radius-sm); background:#e6f2fd; color:#276caa; font-weight:800; }.source-number.cover { background:#f1eafa; color:#7652a7; }.source-title strong,.source-title small { display:block; }.source-title strong { color:#404b57; font-size:14px; }.source-title small { margin-top:2px; color:#87919b; font-size:13px; }.image-slot,.decode-upload { display:grid; place-items:center; width:100%; min-height:210px; margin-top:13px; overflow:hidden; border:1px dashed #c8d2db; border-radius:var(--radius-md); background:var(--c-surface); cursor:pointer; }.image-slot img,.decode-upload img { display:block; width:100%; height:210px; object-fit:contain; }.image-slot>span,.decode-upload>span { display:grid; place-items:center; color:#7d8995; }.image-slot b,.decode-upload b { display:grid; place-items:center; width:40px; height:40px; margin-bottom:9px; border-radius:var(--radius-sm); background:#edf3f8; color:#4d789f; font-size:20px; }.image-slot strong,.image-slot small,.decode-upload strong,.decode-upload small { display:block; }.image-slot strong,.decode-upload strong { color:#56636f; font-size:14px; }.image-slot small,.decode-upload small { margin-top:4px; color:#929ba4; font-size:13px; }.image-slot.filled,.decode-upload.filled { border-style:solid; }.file-info { display:flex; align-items:center; justify-content:space-between; gap:10px; margin-top:8px; }.file-info span { min-width:0; overflow:hidden; color:#5e6874; font-size:13px; white-space:nowrap; text-overflow:ellipsis; }.file-info strong { flex:none; color:#8a949e; font-size:13px; }.source-controls { display:grid; gap:10px; margin-top:14px; padding-top:13px; border-top:1px solid #e5e8ec; }.source-controls label:not(.switch-row),.mix-controls>label:not(.switch-card),.decode-settings>label { display:grid; grid-template-columns:1fr auto; gap:6px 10px; }.source-controls label>span,.mix-controls label>span,.decode-settings label>span { color:#65717d; font-size:13px; font-weight:650; }.source-controls label>strong,.mix-controls label>strong,.decode-settings label>strong { color:#2c6aa3; font-size:13px; }.source-controls input,.mix-controls input,.decode-settings input { grid-column:1/-1; width:100%; accent-color:#2d6eb5; }.switch-row { display:flex; align-items:center; justify-content:space-between; }.switch-row>span { font-size:13px; }
.mix-section,.output-section,.decode-result-section { margin-top:20px; padding-top:19px; border-top:1px solid #e5e8ec; }.preset-grid { display:grid; grid-template-columns:repeat(4,1fr); gap:8px; margin-top:14px; }.preset-grid button { padding:11px 12px; border:1px solid #dde3e8; border-radius:var(--radius-sm); background:#fafcfd; text-align:left; cursor:pointer; }.preset-grid button:hover { border-color:#8eb1d1; background:#f5faff; }.preset-grid strong,.preset-grid span { display:block; }.preset-grid strong { color:#47535f; font-size:14px; }.preset-grid span { margin-top:3px; color:#89939d; font-size:13px; line-height:1.4; }.mix-controls { display:grid; grid-template-columns:repeat(4,1fr); gap:9px; margin-top:11px; }.mix-controls>label { padding:12px; border:1px solid #e0e5e9; border-radius:var(--radius-sm); }.switch-card { display:flex; align-items:center; justify-content:space-between; gap:10px; }.switch-card strong,.switch-card span { display:block; }.switch-card strong { color:#56626e; font-size:13px; }.switch-card span { margin-top:3px; color:#9099a2; font-size:13px!important; font-weight:400!important; }.preview-tabs { display:flex; gap:4px; padding:4px; border-radius:var(--radius-sm); background:#f0f3f6; }.preview-tabs button { border:0; border-radius:var(--radius-xs); padding:6px 9px; background:transparent; color:#747f89; font-size:13px; cursor:pointer; }.preview-tabs button.active { background:var(--c-surface); color:#2c6ba5; box-shadow:0 2px 7px rgba(46,67,85,.08); }.output-stage,.decode-result-stage { position:relative; display:flex; align-items:center; justify-content:center; min-height:400px; max-height:650px; margin-top:13px; overflow:auto; border:1px solid #dfe4e8; border-radius:var(--radius-md); background-color:#eef1f4; background-image:linear-gradient(45deg,#dfe3e8 25%,transparent 25%),linear-gradient(-45deg,#dfe3e8 25%,transparent 25%),linear-gradient(45deg,transparent 75%,#dfe3e8 75%),linear-gradient(-45deg,transparent 75%,#dfe3e8 75%); background-size:24px 24px; background-position:0 0,0 12px,12px -12px,-12px 0; }.output-stage canvas,.decode-result-stage canvas { display:block; max-width:100%; max-height:620px; object-fit:contain; transition:filter .2s; }.output-stage.bright canvas { filter:brightness(1.65) contrast(1.1); }.output-stage.dark canvas { filter:brightness(.55) contrast(1.2); }.empty-output { display:grid; place-items:center; text-align:center; }.empty-output>span { display:grid; place-items:center; width:50px; height:50px; margin-bottom:11px; border-radius:var(--radius-md); background:rgba(255,255,255,.82); color:#5f83a3; font-size:18px; font-weight:800; }.empty-output strong { color:#5f6974; font-size:14px; }.empty-output p { margin:5px 0 0; color:#87919a; font-size:13px; }.processing { position:absolute; right:12px; bottom:12px; padding:7px 10px; border-radius:var(--radius-xs); background:rgba(32,47,63,.78); color: var(--c-on-accent); font-size:13px; }.output-footer { display:flex; align-items:center; justify-content:space-between; gap:12px; margin-top:11px; }.output-footer span,.output-footer strong { display:block; }.output-footer span { color:#89939d; font-size:13px; }.output-footer strong { margin-top:2px; color:#4c5966; font-size:13px; }.output-footer button { border:0; border-radius:var(--radius-sm); padding:10px 14px; background:var(--accent); color: var(--c-on-accent); font-size:14px; font-weight:750; cursor:pointer; }.output-footer button:disabled { opacity:.4; cursor:not-allowed; }
.decode-grid { display:grid; grid-template-columns:minmax(0,1fr) minmax(320px,.75fr); gap:13px; margin-top:15px; }.decode-source,.decode-settings { min-width:0; padding:15px; border:1px solid #e1e5e9; border-radius:var(--radius-md); background:#fbfcfd; }.decode-upload { min-height:330px; margin-top:0; }.decode-upload img { height:330px; }.decode-settings { display:grid; align-content:start; gap:15px; }.settings-title { margin-bottom:2px; }.method-select>span { display:block; margin-bottom:7px; color:#65717d; font-size:13px; font-weight:650; }.method-select>div { display:grid; grid-template-columns:repeat(2,1fr); gap:6px; }.method-select button { border:1px solid #dce2e7; border-radius:var(--radius-xs); padding:8px; background:var(--c-surface); color:#64717d; font-size:13px; cursor:pointer; }.method-select button.active { border-color:#77a5cf; background:#edf6ff; color:#286aa7; }.inline-warning { margin:0; padding:9px 10px; border-radius:var(--radius-sm); background:#fff3df; color:#96631d; font-size:13px; }.privacy-badge { padding:7px 9px; border-radius:var(--radius-xs); background:#e9f7f1; color:#31775b; font-size:13px; font-weight:700; }
.principle-card { display:flex; align-items:flex-start; gap:11px; margin-top:13px; padding:15px 17px; border:1px solid #dfe4e9; border-radius:var(--radius-md); background:var(--c-surface); }.principle-card>span { display:grid; place-items:center; width:38px; height:38px; flex:none; border-radius:var(--radius-sm); background:#edf3f8; color:#53799c; font-size:13px; font-weight:800; }.principle-card strong { color:#4b5763; font-size:14px; }.principle-card p { margin:3px 0 0; color:#7f8993; font-size:13px; line-height:1.6; }
:global(.dark) .mirage-tool { --ink:#eff4f8; --muted:#a2afb9; }:global(.dark) .mode-tabs button,:global(.dark) .workbench-card,:global(.dark) .principle-card { border-color:#3b4b58; background:#172430; }:global(.dark) .mix-section,:global(.dark) .output-section,:global(.dark) .decode-result-section,:global(.dark) .source-controls { border-color:#3c4c59; }:global(.dark) .mode-tabs strong,:global(.dark) .source-title strong,:global(.dark) .file-info span,:global(.dark) .preset-grid strong,:global(.dark) .switch-card strong,:global(.dark) .output-footer strong,:global(.dark) .empty-output strong,:global(.dark) .principle-card strong { color:#e7edf2; }:global(.dark) .mode-tabs button.active,:global(.dark) .source-card,:global(.dark) .decode-source,:global(.dark) .decode-settings,:global(.dark) .preset-grid button,:global(.dark) .mix-controls>label { border-color:#40515f; background:#1d2b37; }:global(.dark) .image-slot,:global(.dark) .decode-upload,:global(.dark) .method-select button,:global(.dark) .step-actions button,:global(.dark) .reset-button,:global(.dark) .settings-title button { border-color:#465662; background:#172430; color:#bbc7cf; }:global(.dark) .method-select button.active { border-color:#5e91bb; background:#20394d; color:#8dc0ec; }:global(.dark) .output-stage,:global(.dark) .decode-result-stage { border-color:#40515e; background-color:#1d2630; background-image:linear-gradient(45deg,#28333e 25%,transparent 25%),linear-gradient(-45deg,#28333e 25%,transparent 25%),linear-gradient(45deg,transparent 75%,#28333e 75%),linear-gradient(-45deg,transparent 75%,#28333e 75%); }:global(.dark) .preview-tabs { background:#202d38; }:global(.dark) .preview-tabs button.active { background:#2a3945; color:#8dc2eb; }@media (max-width:960px) { .mix-controls { grid-template-columns:repeat(2,1fr); }.decode-grid { grid-template-columns:1fr; }}@media (max-width:720px) { .source-grid { grid-template-columns:1fr; }.preset-grid { grid-template-columns:repeat(2,1fr); }.output-stage,.decode-result-stage { min-height:320px; }.step-heading { align-items:flex-start; flex-direction:column; }.step-actions { width:100%; }.step-actions button { flex:1; }.preview-tabs { width:100%; box-sizing:border-box; }.preview-tabs button { flex:1; }}@media (max-width:640px) { .workbench-card { padding:18px 15px; }.mode-tabs { grid-template-columns:1fr; }.mix-controls,.preset-grid { grid-template-columns:1fr; }.file-info { align-items:flex-start; flex-direction:column; }.decode-upload,.decode-upload img { min-height:260px; height:260px; }.output-footer { align-items:stretch; flex-direction:column; }.output-footer button { width:100%; }}</style>
