...
.step-heading>div>span,.settings-title>div>span {
  color: #557b9f;
  font-size: 13px;
  font-weight: 800;
  letter-spacing: .12em;
...
.mode-tabs strong,.mode-tabs small {
  display: block;
}
.mode-tabs strong {
  color: #3a4653;
...
.mode-tabs button.active {
  border-color: #77a4cd;
  background: #f8fbff;
  box-shadow: 0 0 0 3px rgba(45,110,181,.07);
}
...
.workbench-card {
  ...
  background: var(--c-surface);
  box-shadow: 0 11px 28px rgba(40,54,69,.045);
}
.step-heading,.settings-title {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 12px;
}
.step-heading h3,.settings-title h3 {
  margin: 4px 0 0;
  color: var(--ink);
  font-size: 18px;
  font-weight: 800;
...
.source-title strong,.source-title small {
  display: block;
}
.source-title strong {
  color: #404b57;
...
.image-slot img,.decode-upload img {
  display: block;
  width: 100%;
  height: 210px;
  object-fit: contain;
}
.image-slot>span,.decode-upload>span {
  display: grid;
  place-items: center;
  color: #7d8995;
}
.image-slot b,.decode-upload b {
  display: grid;
  place-items: center;
  width: 40px;
  height: 40px;
...
.image-slot strong,.image-slot small,.decode-upload strong,.decode-upload small {
  display: block;
}
.image-slot strong,.decode-upload strong {
  color: #56636f;
  font-size: 14px;
}
.image-slot small,.decode-upload small {
  margin-top: 4px;
  color: #929ba4;
  font-size: 13px;
}
.image-slot.filled,.decode-upload.filled {
  border-style: solid;
}
.file-info {
  display: flex;
...
.source-controls label:not(.switch-row),.mix-controls>label:not(.switch-card),.decode-settings>label {
  display: grid;
  grid-template-columns: 1fr auto;
  gap: 6px 10px;
}
.source-controls label>span,.mix-controls label>span,.decode-settings label>span {
  color: #65717d;
  font-size: 13px;
  font-weight: 650;
}
.source-controls label>strong,.mix-controls label>strong,.decode-settings label>strong {
  color: #2c6aa3;
  font-size: 13px;
}
...
.mix-section,.output-section,.decode-result-section {
  margin-top: 20px;
  padding-top: 19px;
  border-top: 1px solid #e5e8ec;
}
...
.preset-grid strong,.preset-grid span {
  display: block;
}
.preset-grid strong {
  color: #47535f;
...
.switch-card strong,.switch-card span {
  display: block;
}
.switch-card strong {
  color: #56626e;
...
.preview-tabs button.active {
  background: var(--c-surface);
  color: #2c6ba5;
  box-shadow: 0 2px 7px rgba(46,67,85,.08);
}
.output-stage,.decode-result-stage {
  position: relative;
  display: flex;
  align-items: center;
...
.output-stage canvas,.decode-result-stage canvas {
  display: block;
  max-width: 100%;
  max-height: 620px;
  object-fit: contain;
...
.output-stage.bright canvas {
  filter: brightness(1.65) contrast(1.1);
}
.output-stage.dark canvas {
...
.output-footer span,.output-footer strong {
  display: block;
}
.output-footer span {
  color: #89939d;
...
:global(.dark) .mirage-tool {
  --ink: #eff4f8;
  --muted: #a2afb9;
}
:global(.dark) .mode-tabs button,:global(.dark) .workbench-card,:global(.dark) .principle-card {
  border-color: #3b4b58;
  background: #172430;
}
:global(.dark) .mix-section,:global(.dark) .output-section,:global(.dark) .decode-result-section,:global(.dark) .source-controls {
  border-color: #3c4c59;
}
:global(.dark) .mode-tabs strong,:global(.dark) .source-title strong,:global(.dark) .file-info span,:global(.dark) .preset-grid strong,:global(.dark) .switch-card strong,:global(.dark) .output-footer strong,:global(.dark) .empty-output strong,:global(.dark) .principle-card strong {
  color: #e7edf2;
}
:global(.dark) .mode-tabs button.active,:global(.dark) .source-card,:global(.dark) .decode-source,:global(.dark) .decode-settings,:global(.dark) .preset-grid button,:global(.dark) .mix-controls>label {
  border-color: #40515f;
  background: #1d2b37;
}
...
:global(.dark) .method-select button.active {
  border-color: #5e91bb;
  background: #20394d;
  color: #8dc0ec;
}
:global(.dark) .output-stage,:global(.dark) .decode-result-stage {
  border-color: #40515e;
  background-color: #1d2630;
  background-image: linear-gradient(45deg,#28333e 25%,transparent 25%),linear-gradient(-45deg,#28333e 25%,transparent 25%),linear-gradient(45deg,transparent 75%,#28333e 75%),linear-gradient(-45deg,transparent 75%,#28333e 75%);
}
:global(.dark) .preview-tabs {
  background: #202d38;
}
:global(.dark) .preview-tabs button.active {
  background: #2a3945;
  color: #8dc2eb;
}
...
@media (max-width:640px) {
  ...
  .decode-upload,.decode-upload img {
    min-height: 260px;
    height: 260px;
  }
  .output-footer {
