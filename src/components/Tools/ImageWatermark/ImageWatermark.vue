...
<template>
  <div class="watermark-page flex flex-col mt-3 flex-1">
    <ToolHero summary="边调参数，边看最终成片">
      <template #metrics>
        <MetricsBar :items="[{ label: '定位模式', value: '6' }, { label: '导出格式', value: '3' }, { label: '图片上传', value: '0' }]" />
      </template>
    </ToolHero>
...
.upload-card, .preview-card, .settings-card, .feature-strip article { border: 1px solid var(--c-border); background: var(--c-surface); box-shadow: 0 16px 40px rgba(15, 23, 42, .06); }
...
.upload-icon { display: grid; place-items: center; width: 72px; height: 72px; margin-bottom: 18px; border-radius: var(--radius-card); color: var(--c-on-accent); font-size: 34px; background: linear-gradient(145deg, #2563eb, #0d9488); box-shadow: 0 14px 28px rgba(37,99,235,.25); }
.upload-card h3, .card-heading h3 { margin: 0; color: var(--c-text-primary); }.upload-card h3 { font-size: 24px; }.upload-card p { max-width: 560px; margin: 10px auto 22px; color: var(--c-text-secondary); line-height: 1.7; }.upload-actions { display: flex; gap: 10px; }
...
.card-heading { display: flex; align-items: center; justify-content: space-between; gap: 12px; margin-bottom: 16px; }.card-heading .eyebrow { margin-bottom: 3px; }.header-actions { display: flex; gap: 8px; }.text-button { border: 0; background: none; color: var(--c-text-secondary); cursor: pointer; font-size: 12px; }
.canvas-stage { position: relative; min-height: 420px; display: grid; place-items: center; padding: 18px; overflow: auto; border-radius: var(--radius-lg); background-color: #e2e8f0; background-image: linear-gradient(45deg,#cbd5e1 25%,transparent 25%),linear-gradient(-45deg,#cbd5e1 25%,transparent 25%),linear-gradient(45deg,transparent 75%,#cbd5e1 75%),linear-gradient(-45deg,transparent 75%,#cbd5e1 75%); background-size: 24px 24px; background-position: 0 0,0 12px,12px -12px,-12px 0; }
.canvas-stage canvas { display: block; max-width: 100%; max-height: 68vh; object-fit: contain; box-shadow: 0 22px 44px rgba(15,23,42,.22); }.rendering-badge { position: absolute; right: 12px; bottom: 12px; padding: 6px 10px; border-radius: var(--radius-full); color: var(--c-on-accent); background: rgba(15,23,42,.75); font-size: 11px; }
.source-image { display: none; }.image-meta { display: grid; grid-template-columns: 1.5fr repeat(3, 1fr); margin-top: 14px; border: 1px solid var(--c-border); border-radius: var(--radius-md); overflow: hidden; }.image-meta div { min-width: 0; padding: 11px 13px; border-right: 1px solid var(--c-border); }.image-meta div:last-child { border: 0; }.image-meta span, .image-meta strong { display: block; }.image-meta span { color: var(--c-text-muted); font-size: 10px; }.image-meta strong { margin-top: 3px; overflow: hidden; color: var(--c-text-strong); font-size: 12px; text-overflow: ellipsis; white-space: nowrap; }
.field-group { margin-bottom: 15px; }.field-group > label, .slider-field > label { display: flex; justify-content: space-between; margin-bottom: 7px; color: var(--c-text-body); font-size: 12px; font-weight: 700; }.field-group label small, .toggle-row small { color: var(--c-text-muted); font-weight: 400; }.two-columns { display: grid; grid-template-columns: 1fr 1fr; gap: 0 10px; }.two-columns :deep(.el-input-number), .two-columns :deep(.el-select) { width: 100%; }.color-row { display: flex; align-items: center; gap: 10px; height: 32px; }.color-row code { color: var(--c-text-secondary); font-size: 12px; }.slider-field { margin-bottom: 12px; }.slider-field label strong { color: var(--studio-accent); }
.position-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 7px; }.position-grid button { display: flex; align-items: center; justify-content: center; gap: 5px; padding: 8px 5px; border: 1px solid var(--c-border); border-radius: var(--radius-sm); color: var(--c-text-secondary); background: var(--c-surface-subtle); cursor: pointer; }.position-grid button.active { border-color: #60a5fa; color: #1d4ed8; background: #eff6ff; box-shadow: inset 0 0 0 1px #93c5fd; }
.toggle-row { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin: 14px 0; }.toggle-row label { display: flex; align-items: center; justify-content: space-between; gap: 8px; padding: 10px; border: 1px solid var(--c-border); border-radius: var(--radius-md); color: var(--c-text-strong); font-size: 12px; }.toggle-row span, .toggle-row small { display: block; }
.export-box { margin-top: 16px; padding: 14px; border-radius: var(--radius-lg); background: var(--c-surface-subtle); }.download-button { width: 100%; }
.feature-strip { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; }.feature-strip article { display: flex; gap: 12px; padding: 17px; border-radius: var(--radius-lg); }.feature-strip b { display: grid; place-items: center; flex: 0 0 34px; height: 34px; border-radius: var(--radius-sm); color: #ea580c; background: #fff7ed; font-size: 11px; }.feature-strip strong { color: #1e293b; font-size: 13px; }.feature-strip p { margin: 4px 0 0; color: var(--c-text-secondary); font-size: 11px; line-height: 1.55; }
:global(html.dark .watermark-page .upload-card), :global(html.dark .watermark-page .preview-card), :global(html.dark .watermark-page .settings-card), :global(html.dark .watermark-page .feature-strip article) { border-color: var(--c-border); background-color: var(--c-surface); box-shadow: none; }
:global(html.dark .watermark-page .upload-card.dragging), :global(html.dark .watermark-page .export-box) { background: var(--c-surface-subtle); }:global(html.dark .watermark-page .upload-card h3), :global(html.dark .watermark-page .card-heading h3), :global(html.dark .watermark-page .feature-strip strong) { color: #f8fafc; }:global(html.dark .watermark-page .upload-card p), :global(html.dark .watermark-page .feature-strip p), :global(html.dark .watermark-page .field-group > label), :global(html.dark .watermark-page .slider-field > label) { color: var(--c-text-muted); }
:global(html.dark .watermark-page .canvas-stage) { background-color: var(--c-surface-subtle); background-image: linear-gradient(45deg,#1e293b 25%,transparent 25%),linear-gradient(-45deg,#1e293b 25%,transparent 25%),linear-gradient(45deg,transparent 75%,#1e293b 75%),linear-gradient(-45deg,transparent 75%,#1e293b 75%); }:global(html.dark .watermark-page .image-meta), :global(html.dark .watermark-page .image-meta div), :global(html.dark .watermark-page .position-grid button), :global(html.dark .watermark-page .toggle-row label) { border-color: var(--c-border); }:global(html.dark .watermark-page .image-meta strong), :global(html.dark .watermark-page .toggle-row label) { color: var(--c-text-primary); }:global(html.dark .watermark-page .position-grid button) { color: var(--c-text-muted); background: var(--c-surface-subtle); }:global(html.dark .watermark-page .position-grid button.active) { color: #93c5fd; background: #172554; }:global(html.dark .watermark-page .feature-strip b) { background: #431407; }@media (max-width: 1080px) { .studio-grid { grid-template-columns: 1fr; }.settings-card { position: static; }.canvas-stage { min-height: 360px; }}@media (max-width: 720px) { .watermark-page { gap: 12px; }.upload-card { min-height: 360px; padding: 36px 18px; }.preview-card, .settings-card { padding: 15px; border-radius: var(--radius-card); }.card-heading { align-items: flex-start; }.header-actions { flex-wrap: wrap; justify-content: flex-end; }.canvas-stage { min-height: 280px; padding: 10px; }.image-meta { grid-template-columns: 1fr 1fr; }.image-meta div:nth-child(2) { border-right: 0; }.image-meta div:nth-child(-n+2) { border-bottom: 1px solid var(--c-border); }.feature-strip { grid-template-columns: 1fr; }.two-columns, .toggle-row { grid-template-columns: 1fr; }.upload-actions { flex-direction: column; width: 100%; }.upload-actions :deep(.el-button) { width: 100%; margin-left: 0; }}</style>
