    ...
    <ToolHero summary="每一格，都完整覆盖原图">
      <template #metrics>
        <div class="hero-metrics">
          <div><span>{{ sourceUrl ? `${sliceCount} 张切片` : '自由网格' }}</span><strong>{{ sourceUrl ? `${state.rows} × ${state.columns}` : 'R × C' }}</strong></div>
        </div>
      </template>
    </ToolHero>
