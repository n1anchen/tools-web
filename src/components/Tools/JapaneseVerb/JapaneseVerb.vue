...
<template>
  <div class="verb-tool flex flex-col mt-3 flex-1">
    <ToolHero summary="从任意活用形，反推动词原形">
      <template #metrics>
        <MetricsBar :items="[{ label: '识别原形', value: activeResult?.dictionary || '—' }, { label: '动词类型', value: activeResult?.kindLabel || '等待输入' }, { label: '候选数量', value: results.length }]" />
      </template>
    </ToolHero>
...
.search-heading span,.result-kicker,.reference-heading>span { color:#7a64aa; font-size:13px; font-weight:800; letter-spacing:.12em; }
.search-card,.reference-card,.onboarding-card,.result-workbench,.notice-card { margin-top:14px; border:1px solid #e1e2e9; border-radius:19px; background:var(--c-surface); box-shadow:0 10px 26px rgba(48,43,64,.045); }.search-card { padding:21px; }.search-heading { display:flex; align-items:center; justify-content:space-between; }.search-heading h3,.reference-heading h3 { margin:4px 0 0; color:var(--ink); font-size:18px; font-weight:800; }.text-button { border:0; background:transparent; color:#7657bc; font-size:13px; font-weight:700; cursor:pointer; }.search-shell { display:flex; align-items:center; gap:8px; margin-top:15px; padding:6px; border:1px solid #d7d5e1; border-radius:var(--radius-md); background:#fbfbfd; }.search-shell:focus-within { border-color:#9a82d4; box-shadow:0 0 0 3px rgba(109,79,194,.08); }.search-mark { display:grid; place-items:center; width:42px; height:42px; flex:none; border-radius:var(--radius-sm); background:#eee9fc; color:#7054b3; font-size:16px; font-weight:800; }.search-shell input { flex:1; min-width:0; border:0; outline:0; background:transparent; color:#302d39; font-size:17px; font-family:"Hiragino Sans","Yu Gothic",sans-serif; }.search-shell>button { flex:none; border:0; border-radius:var(--radius-sm); padding:12px 17px; background:var(--accent); color: var(--c-on-accent); font-size:14px; font-weight:750; cursor:pointer; }.search-shell>button:disabled { opacity:.42; cursor:not-allowed; }.search-tip { margin:8px 2px 0; color:#898493; font-size:13px; }.quick-inputs { display:grid; gap:9px; margin-top:15px; padding-top:14px; border-top:1px solid #eceaf0; }.quick-inputs>div { display:flex; align-items:center; flex-wrap:wrap; gap:7px; }.quick-inputs span { width:64px; color:#777184; font-size:13px; font-weight:700; }.quick-inputs button { border:1px solid #dedbe7; border-radius:var(--radius-xs); padding:6px 10px; background:var(--c-surface); color:#5d5768; font-size:14px; cursor:pointer; }.quick-inputs button:hover { border-color:#a996d7; color:#6549aa; background:#faf8ff; }
...
.result-workbench { display:grid; grid-template-columns:220px minmax(0,1fr); overflow:hidden; }.candidate-panel { padding:16px 12px; border-right:1px solid #e5e3eb; background:#f9f8fb; }.panel-label { padding:2px 7px 10px; color:#888292; font-size:13px; font-weight:800; letter-spacing:.08em; }.candidate-panel>button { display:grid; grid-template-columns:auto 1fr auto; align-items:center; gap:9px; width:100%; margin-bottom:7px; padding:11px 9px; border:1px solid transparent; border-radius:var(--radius-md); background:transparent; text-align:left; cursor:pointer; }.candidate-panel>button.active { border-color:#b8a8df; background:var(--c-surface); box-shadow:0 5px 15px rgba(77,61,113,.07); }.candidate-index { color:#aaa3b1; font:700 12px ui-monospace,SFMono-Regular,Menlo,monospace; }.candidate-panel strong,.candidate-panel small { display:block; }.candidate-panel strong { color:#373341; font-size:15px; }.candidate-panel small { margin-top:2px; color:#898290; font-size:13px; }.confidence { color:#7056ae; font-size:13px; font-weight:750; }.candidate-note { margin:12px 7px 0; padding-top:12px; border-top:1px solid #e3e0e8; color:#918a98; font-size:13px; line-height:1.55; }
.forms-panel { min-width:0; padding:22px; }.result-header { display:flex; align-items:flex-start; justify-content:space-between; gap:18px; }.dictionary-line { display:flex; align-items:center; gap:10px; margin-top:4px; }.dictionary-line h3 { margin:0; color:#2e2a36; font-size:26px; font-family:"Hiragino Sans","Yu Gothic",sans-serif; }.dictionary-line span { padding:5px 9px; border-radius:var(--radius-xs); background:#eeeafd; color:#654aa7; font-size:13px; font-weight:700; }.result-header p { margin:7px 0 0; color:#7d7685; font-size:13px; }.result-actions { display:flex; align-items:center; gap:8px; flex-wrap:wrap; justify-content:flex-end; }.confidence-badge { padding:7px 9px; border-radius:var(--radius-xs); background:#fff3df; color:#956522; font-size:13px; font-weight:700; }.confidence-badge.high { background:#e7f7ef; color:#287555; }.result-actions button { border:1px solid #d8d0e8; border-radius:var(--radius-xs); padding:7px 10px; background:var(--c-surface); color:#684da9; font-size:13px; font-weight:700; cursor:pointer; }.matched-banner { display:flex; align-items:center; flex-wrap:wrap; gap:8px; margin-top:17px; padding:11px 13px; border-radius:var(--radius-md); background:#fff8e9; color:#8b7047; font-size:13px; }.matched-banner strong { color:#714d17; font-size:15px; }.matched-banner span:last-child { color:#9b8667; }.form-tabs { display:flex; gap:5px; margin-top:17px; padding:4px; overflow-x:auto; border-radius:var(--radius-sm); background:#f1eff4; }.form-tabs button { flex:none; border:0; border-radius:var(--radius-xs); padding:8px 12px; background:transparent; color:#746d7a; font-size:13px; font-weight:700; cursor:pointer; }.form-tabs button.active { background:var(--c-surface); color:#674aae; box-shadow:0 2px 8px rgba(65,52,85,.08); }.form-grid { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:10px; margin-top:13px; }.form-card { min-width:0; padding:14px; border:1px solid #e4e1e8; border-radius:var(--radius-md); background:var(--c-surface); }.form-card-heading { display:flex; align-items:flex-start; justify-content:space-between; gap:10px; }.form-card-heading strong,.form-card-heading span { display:block; }.form-card-heading strong { color:#433e49; font-size:14px; }.form-card-heading span { margin-top:3px; color:#96909a; font-size:13px; line-height:1.4; }.form-card-heading button { border:0; background:transparent; color:#8268bd; font-size:13px; cursor:pointer; }.form-values { display:flex; flex-wrap:wrap; gap:7px; margin-top:12px; }.form-chip { border:1px solid #ddd9e4; border-radius:var(--radius-xs); padding:7px 9px; background:#faf9fb; color:#3f3a45; font-size:15px; font-family:"Hiragino Sans","Yu Gothic",sans-serif; cursor:pointer; }.form-chip:hover { border-color:#aa98d2; }.form-chip.matched { border-color:#e6ae48; background:#fff4d9; color:#744a09; box-shadow:0 0 0 2px rgba(230,174,72,.12); }.form-note { margin:9px 0 0; color:#9a7380; font-size:13px; }
.onboarding-card { display:grid; grid-template-columns:minmax(260px,.8fr) minmax(0,1.2fr); overflow:hidden; }.onboarding-main { display:grid; place-items:center; align-content:center; padding:28px; text-align:center; background:linear-gradient(135deg,#faf8ff,#f4f0ff); }.large-glyph { color:#7256b7; font:800 26px ui-monospace,SFMono-Regular,Menlo,monospace; }.onboarding-main h3 { margin:13px 0 5px; color:#403a49; font-size:17px; }.onboarding-main p { margin:0; max-width:360px; color:#817a87; font-size:13px; line-height:1.65; }.rule-preview { display:grid; grid-template-columns:repeat(3,1fr); gap:10px; padding:20px; }.rule-preview div { padding:15px; border:1px solid #e5e2e9; border-radius:var(--radius-md); }.rule-preview span { color:#917dc0; font-size:13px; font-weight:800; }.rule-preview strong { display:block; margin-top:7px; color:#4d4754; font-size:14px; }.rule-preview p { margin:5px 0 0; color:#8a838d; font-size:13px; line-height:1.55; }
.reference-card { padding:20px; }.reference-grid { display:grid; grid-template-columns:repeat(5,1fr); gap:8px; margin-top:14px; }.reference-grid div { padding:12px; border:1px solid #e4e1e7; border-radius:var(--radius-md); background:#fbfafc; }.reference-grid strong,.reference-grid span,.reference-grid small { display:block; }.reference-grid strong { color:#514b58; font-size:14px; }.reference-grid span { margin-top:6px; color:#684ca9; font-size:15px; font-weight:750; }.reference-grid small { margin-top:4px; color:#908995; font-size:13px; }.reference-grid .exception { border-color:#ecd5a9; background:#fffbf1; }.reference-grid .exception span { color:#9a681e; }
:global(.dark) .verb-tool { --ink:#f1edf4; --muted:#a9a1ae; }:global(.dark) .search-card,:global(.dark) .reference-card,:global(.dark) .onboarding-card,:global(.dark) .result-workbench,:global(.dark) .notice-card { border-color:#3e4656; background:#1b2637; }:global(.dark) .quick-inputs,:global(.dark) .candidate-panel,:global(.dark) .candidate-note { border-color:#41495a; }:global(.dark) .search-shell input,:global(.dark) .candidate-panel strong,:global(.dark) .dictionary-line h3,:global(.dark) .form-card-heading strong,:global(.dark) .form-chip,:global(.dark) .reference-grid strong { color:#ece7ef; }:global(.dark) .search-shell,:global(.dark) .candidate-panel,:global(.dark) .form-tabs,:global(.dark) .onboarding-main { border-color:#41495a; background:#202b3d; }:global(.dark) .quick-inputs button,:global(.dark) .candidate-panel>button.active,:global(.dark) .result-actions button,:global(.dark) .form-card,:global(.dark) .form-tabs button.active,:global(.dark) .form-chip,:global(.dark) .reference-grid div { border-color:#465063; background:#202c3f; color:#c7becd; }:global(.dark) .form-chip.matched { border-color:#b98a38; background:#3b3120; color:#ffd98f; }:global(.dark) .matched-banner { background:#352f24; }:global(.dark) .onboarding-main h3 { color:#ece6ef; }:global(.dark) .rule-preview div { border-color:#41495a; }:global(.dark) .reference-grid .exception { border-color:#705a35; background:#342e24; }@media (max-width:900px) {.result-workbench { grid-template-columns:1fr; }.candidate-panel { display:flex; gap:7px; overflow-x:auto; border-right:0; border-bottom:1px solid #e5e3eb; }.panel-label,.candidate-note { display:none; }.candidate-panel>button { flex:0 0 190px; margin:0; }.reference-grid { grid-template-columns:repeat(3,1fr); }.onboarding-card { grid-template-columns:1fr; }}@media (max-width:640px) { .search-card,.forms-panel,.reference-card { padding:18px 15px; }.search-shell { flex-wrap:wrap; }.search-shell input { min-width:calc(100% - 58px); }.search-shell>button { width:100%; }.quick-inputs>div { align-items:flex-start; }.quick-inputs span { width:100%; }.result-header { flex-direction:column; }.result-actions { justify-content:flex-start; }.form-grid { grid-template-columns:1fr; }.rule-preview,.reference-grid { grid-template-columns:1fr; }.onboarding-main { padding:24px 17px; }}</style>
