...
:global(html.dark .time-page .note-card strong) { color: #fdba74; }
:global(html.dark .time-page .note-card p) { color: #fed7aa; }

@media (max-width: 920px) {
  .clock-card { align-items: flex-start; flex-wrap: wrap; }
  .clock-stamps { order: 3; width: 100%; }
  .clock-stamps button { flex: 1; }
  .converter-grid { grid-template-columns: 1fr; }
}
@media (max-width: 640px) {
  .clock-card, .converter-card { padding: 18px; border-radius: 19px; }
  .clock-main strong { font-size: 18px; }
  .clock-stamps { display: grid; grid-template-columns: 1fr 1fr; }
  .clock-stamps strong { font-size: 11px; }
  .timestamp-input { grid-template-columns: 1fr; }
  .date-options { align-items: flex-start; flex-direction: column; }
  .preset-row.compact { justify-content: flex-start; }
  .timestamp-results button { align-items: flex-start; flex-wrap: wrap; }
  .timestamp-results button > div { width: 82px; }
  .timestamp-results strong { flex-basis: calc(100% - 115px); }
}
</style>
