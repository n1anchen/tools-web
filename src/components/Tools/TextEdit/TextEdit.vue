...
.document-bar,.editor-card,.insight-card,.template-card,.source-card {
...
.document-state strong,.document-state span {
  display:block
}
...
.workspace-grid {
  display: grid;
  grid-template-columns: minmax(0,1.65fr) minmax(280px,.58fr);
  gap:18px
}
.editor-card,.insight-card,.template-card,.source-card {
  padding:22px
}
.card-heading,.source-heading {
  display: flex;
...
.card-heading h3,.source-heading h3 {
  margin: 0;
...
.dark .editor-shell :deep(.w-e-toolbar) {
  border-color: #334155;
  background: #0f172a;
  color:#cbd5e1
}
.dark .editor-shell :deep(.w-e-text-container),.dark .editor-shell :deep(.w-e-scroll) {
  background: #111827;
  color:#e2e8f0
}
.dark .editor-shell :deep(.w-e-text-placeholder) {
  color: var(--c-text-secondary)
}
