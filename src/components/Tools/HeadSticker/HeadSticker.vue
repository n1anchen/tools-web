...
<template>
  <div class="sticker-tool flex flex-col mt-3 flex-1">
    <ToolHero summary="选一张底图，开始自由接头">
      <template #metrics>
        <MetricsBar :items="[{ label: '底图', value: hasBackground ? `${originalWidth} × ${originalHeight}` : '未选择' }, { label: '贴纸数量', value: stickerCount }, { label: '导出', value: (exportFormat.toUpperCase()) + ' · ' + (exportScale) + '×' }]" />
      </template>
    </ToolHero>
...
<style scoped>
.sticker-tool { --accent:#7157d9; --ink:#292b38; --muted:#737789; }.panel-heading span { color:#7665a5; font-size:13px; font-weight:800; letter-spacing:.12em; }
.workspace-card { display:grid; grid-template-columns:280px minmax(0,1fr); margin-top:14px; overflow:hidden; border:1px solid #e1e3e9; border-radius:var(--radius-card); background:var(--c-surface); box-shadow:0 11px 28px rgba(47,43,63,.05); }.asset-panel { padding:18px; border-right:1px solid #e4e5ea; background:#fafafb; }.panel-heading h3 { margin:4px 0 0; color:var(--ink); font-size:18px; }.background-upload,.custom-upload { display:flex; align-items:center; gap:11px; width:100%; margin-top:15px; padding:12px; border:1px solid #dcd9e7; border-radius:var(--radius-md); background:var(--c-surface); text-align:left; cursor:pointer; }.upload-icon { display:grid; place-items:center; width:38px; height:38px; flex:none; border-radius:var(--radius-sm); background:#eee9fc; color:#7054b6; font-weight:800; }.background-upload strong,.background-upload small,.custom-upload strong,.custom-upload small { display:block; }.background-upload strong,.custom-upload strong { color:#494351; font-size:14px; }.background-upload small,.custom-upload small { max-width:170px; margin-top:3px; overflow:hidden; color:#8c8590; font-size:13px; white-space:nowrap; text-overflow:ellipsis; }.group-tabs { display:flex; gap:5px; margin-top:16px; overflow-x:auto; }.group-tabs button { display:flex; align-items:center; gap:5px; flex:none; padding:6px 8px; border:1px solid transparent; border-radius:var(--radius-sm); background:transparent; color:#716a77; font-size:13px; cursor:pointer; }.group-tabs button.active { border-color:#c5b9e5; background:var(--c-surface); color:#684cae; }.group-tabs img { width:24px; height:24px; object-fit:contain; }.sticker-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:7px; margin-top:10px; }.sticker-grid button { min-width:0; padding:6px; border:1px solid #e0dee6; border-radius:var(--radius-sm); background:var(--c-surface); cursor:grab; }.sticker-grid button:hover { border-color:#a996d8; transform:translateY(-1px); }.sticker-grid img { display:block; width:100%; aspect-ratio:1; object-fit:contain; }.sticker-grid span { display:block; margin-top:4px; overflow:hidden; color:#77717c; font-size:13px; white-space:nowrap; text-overflow:ellipsis; }.custom-upload { margin-top:12px; border-style:dashed; }.custom-upload>span { display:grid; place-items:center; width:36px; height:36px; border-radius:var(--radius-sm); background:#f2f0f6; color:#8069b6; font-size:19px; }
.canvas-panel { min-width:0; }.canvas-toolbar { display:flex; align-items:center; justify-content:space-between; gap:12px; min-height:58px; padding:10px 16px; border-bottom:1px solid #e5e6eb; }.tool-status { display:flex; align-items:center; gap:8px; min-width:0; }.tool-status>span { width:9px; height:9px; flex:none; border-radius:50%; background:#c4c7ce; }.tool-status>span.active { background:#35a779; box-shadow:0 0 0 4px rgba(53,167,121,.12); }.tool-status strong { overflow:hidden; color:#55505c; font-size:13px; white-space:nowrap; text-overflow:ellipsis; }.selected-tools { display:flex; flex-wrap:wrap; justify-content:flex-end; gap:5px; }.selected-tools.disabled { pointer-events:none; opacity:.38; }.selected-tools button { min-width:32px; height:32px; border:1px solid #dcdde4; border-radius:var(--radius-xs); background:var(--c-surface); color:#5d5865; font-size:13px; font-weight:700; cursor:pointer; }.selected-tools button.danger { color:#bd4953; }.stage-controls { display:flex; justify-content:flex-end; align-items:center; gap:5px; padding:9px 16px; background:#f8f9fb; }.stage-controls>span { margin-right:3px; color:#7f8390; font-size:13px; }.stage-controls button { border:1px solid transparent; border-radius:var(--radius-xs); padding:5px 8px; background:transparent; color:#737782; font-size:13px; cursor:pointer; }.stage-controls button.active { border-color:#d6d7df; background:var(--c-surface); color:#5e4c9c; }.canvas-stage { position:relative; display:flex; align-items:center; justify-content:center; min-height:520px; max-height:720px; padding:24px; overflow:auto; box-sizing:border-box; }.canvas-stage.checker { background-color:#eef0f4; background-image:linear-gradient(45deg,#dde0e7 25%,transparent 25%),linear-gradient(-45deg,#dde0e7 25%,transparent 25%),linear-gradient(45deg,transparent 75%,#dde0e7 75%),linear-gradient(-45deg,transparent 75%,#dde0e7 75%); background-size:24px 24px; background-position:0 0,0 12px,12px -12px,-12px 0; }.canvas-stage.light { background:#f5f6f8; }.canvas-stage.dark { background:#20242d; }.canvas-stage :deep(.canvas-container) { flex:none; box-shadow:0 14px 34px rgba(29,31,39,.18); }.canvas-stage canvas.hidden,.canvas-stage :deep(.canvas-container:has(canvas.hidden)) { display:none!important; }.empty-stage { position:absolute; inset:24px; display:grid; place-items:center; align-content:center; border:1px dashed #bfc3ce; border-radius:var(--radius-lg); background:rgba(255,255,255,.78); text-align:center; }.empty-glyph { font-size:34px; }.empty-stage strong { margin-top:9px; color:#555966; font-size:15px; }.empty-stage p { margin:5px 0 13px; color:#858995; font-size:13px; }.empty-stage button { border:0; border-radius:var(--radius-sm); padding:9px 13px; background:var(--accent); color:white; font-size:13px; font-weight:700; cursor:pointer; }.canvas-footer { display:flex; align-items:center; gap:18px; min-height:55px; padding:9px 16px; border-top:1px solid #e5e6eb; }.canvas-footer div span,.canvas-footer div strong { display:block; }.canvas-footer div span { color:#8a8e99; font-size:13px; }.canvas-footer div strong { margin-top:2px; color:#505563; font-size:13px; }.canvas-footer button { margin-left:auto; border:0; background:transparent; color:#7560ac; font-size:13px; font-weight:700; cursor:pointer; }.canvas-footer button+button { margin-left:0; color:#aa5b64; }
.export-card { display:flex; align-items:center; justify-content:space-between; gap:18px; margin-top:14px; padding:16px 18px; border:1px solid #e1e3e9; border-radius:17px; background:var(--c-surface); }.export-card>div:first-child { display:flex; align-items:center; gap:11px; }.export-icon { display:grid; place-items:center; width:42px; height:42px; border-radius:var(--radius-sm); background:#eaf5f1; color:#28765c; font-weight:800; }.export-card strong { color:#484b57; font-size:14px; }.export-card p { margin:3px 0 0; color:#818591; font-size:13px; }.export-options { display:flex; align-items:center; gap:8px; }.export-options label { display:flex; align-items:center; gap:5px; color:#737784; font-size:13px; }.export-options select { height:36px; border:1px solid #d9dce3; border-radius:var(--radius-xs); padding:0 8px; background:var(--c-surface); color:#515662; }.export-options>span { min-width:110px; color:#777b87; font-size:13px; text-align:center; }.export-options>button { border:0; border-radius:var(--radius-sm); padding:10px 14px; background:var(--accent); color: var(--c-on-accent); font-size:14px; font-weight:750; cursor:pointer; }.export-options>button:disabled { opacity:.4; cursor:not-allowed; }
:global(.dark) .sticker-tool { --ink:#f1edf5; --muted:#aaa3b1; }:global(.dark) .workspace-card,:global(.dark) .export-card { border-color:#3f4756; background:#1b2637; }:global(.dark) .asset-panel,:global(.dark) .canvas-toolbar,:global(.dark) .canvas-footer { border-color:#414958; }:global(.dark) .background-upload strong,:global(.dark) .custom-upload strong,:global(.dark) .tool-status strong,:global(.dark) .canvas-footer div strong,:global(.dark) .export-card strong { color:#ece7ef; }:global(.dark) .asset-panel,:global(.dark) .stage-controls { background:#202b3d; }:global(.dark) .background-upload,:global(.dark) .custom-upload,:global(.dark) .group-tabs button.active,:global(.dark) .sticker-grid button,:global(.dark) .selected-tools button,:global(.dark) .stage-controls button.active,:global(.dark) .export-options select { border-color:#465061; background:#1b2739; color:#c9c1ce; }:global(.dark) .empty-stage { border-color:#566173; background:rgba(27,38,55,.88); }@media (max-width:1000px) { .workspace-card { grid-template-columns:230px minmax(0,1fr); }.sticker-grid { grid-template-columns:repeat(2,1fr); }.canvas-toolbar { align-items:flex-start; flex-direction:column; }.selected-tools { justify-content:flex-start; }}@media (max-width:760px) { .workspace-card { grid-template-columns:1fr; }.asset-panel { border-right:0; border-bottom:1px solid #e4e5ea; }.sticker-grid { display:flex; overflow-x:auto; }.sticker-grid button { flex:0 0 84px; }.canvas-stage { min-height:360px; padding:14px; }.export-card { align-items:flex-start; flex-direction:column; }.export-options { width:100%; flex-wrap:wrap; }.export-options>button { width:100%; }.canvas-footer { flex-wrap:wrap; }.canvas-footer button { margin-left:0; }}@media (max-width:640px) {.canvas-stage { min-height:330px; }.stage-controls { justify-content:flex-start; }.export-options>span { min-width:0; margin-left:auto; }}</style>
