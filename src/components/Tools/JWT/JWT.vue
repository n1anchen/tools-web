...
.security-banner, .decoder-card, .segments-card, .json-card, .claims-card, .status-card, .algorithm-card { border: 1px solid #e2e8f0; border-radius: 21px; background: #fff; box-shadow: 0 12px 35px rgb(15 23 42 / 6%); }
...
.card-heading h2, .section-heading h3 { margin: 5px 0 0; color: #0f172a; }
.card-heading h2 { font-size: 20px; }
.section-heading h3 { font-size: 18px; }
.decoder-card :deep(.el-textarea__inner) { margin-top: 17px; padding: 16px; border: 1px solid #e2e8f0; border-radius: 15px; box-shadow: none; color: #334155; background: #f8fafc; font: 13px/1.7 ui-monospace, SFMono-Regular, Menlo, monospace; }
.input-meta { justify-content: flex-start; flex-wrap: wrap; margin-top: 11px; color: #64748b; font-size: 11px; }
...
.status-card > strong { display: block; margin-top: 7px; color: #0f172a; font-size: 21px; }
.status-card p { margin: 3px 0 0; color: #64748b; font-size: 12px; }
.algorithm-card { display: grid; grid-template-columns: repeat(2, 1fr); align-items: center; gap: 10px; }
.algorithm-card > div { display: flex; flex-direction: column; padding: 11px; border-radius: 13px; background: #f8fafc; }
.algorithm-card span { color: #94a3b8; font-size: 10px; }
.algorithm-card strong { margin-top: 4px; color: #334155; font: 700 15px ui-monospace, monospace; }
...
.segments-grid article { min-width: 0; padding: 14px; border: 1px solid #e2e8f0; border-top-width: 4px; border-radius: 14px; background: #f8fafc; }
...
.segments-grid strong { color: #334155; }
.segments-grid span, .segments-grid small { color: #94a3b8; font-size: 10px; }
.segments-grid code { display: block; margin: 11px 0; color: #475569; overflow: hidden; font: 11px ui-monospace, monospace; white-space: nowrap; }
...
.json-card pre { min-height: 185px; max-height: 360px; margin: 17px 0 0; padding: 16px; border-radius: 15px; color: #334155; overflow: auto; background: #f8fafc; font: 12px/1.65 ui-monospace, SFMono-Regular, Menlo, monospace; }
.claims-list { display: grid; gap: 8px; margin-top: 17px; }
.claims-list > div { display: grid; grid-template-columns: 52px 140px minmax(0, 1fr); align-items: center; gap: 12px; padding: 11px 13px; border: 1px solid #e2e8f0; border-radius: 13px; background: #f8fafc; }
.claims-list code { color: #7c3aed; font-weight: 800; }
.claims-list > div > div { display: flex; min-width: 0; flex-direction: column; }
.claims-list strong { color: #334155; overflow-wrap: anywhere; font-size: 12px; }
.claims-list small { color: #94a3b8; font-size: 10px; }
.claim-value { text-align: right; }
.empty-claims { margin-top: 17px; padding: 20px; border-radius: 14px; color: #94a3b8; background: #f8fafc; text-align: center; }

:global(html.dark .jwt-page .security-banner), :global(html.dark .jwt-page .decoder-card), :global(html.dark .jwt-page .segments-card), :global(html.dark .jwt-page .json-card), :global(html.dark .jwt-page .claims-card), :global(html.dark .jwt-page .status-card), :global(html.dark .jwt-page .algorithm-card) { border-color: #334155; color: #e2e8f0; background: #1e293b; box-shadow: none; }
...
:global(html.dark .jwt-page .algorithm-card strong) { color: #e2e8f0; }
:global(html.dark .jwt-page .segments-grid code), :global(html.dark .jwt-page .segments-grid span), :global(html.dark .jwt-page .segments-grid small) { color: #94a3b8; }
