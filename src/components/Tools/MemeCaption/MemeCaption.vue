    ...
    <ToolHero summary="把一句话，放到最有戏的位置">
      <template #metrics>
        <div class="hero-metrics">
          <div><span>输出宽度</span><strong>{{ outputDimensions.width || '—' }}</strong></div>
          <div><span>字幕行数</span><strong>{{ renderedLines.length || '—' }}</strong></div>
          <div><span>导出格式</span><strong>{{ state.format.toUpperCase() }}</strong></div>
        </div>
      </template>
    </ToolHero>
