...
async function copyPng() {
  ...
  try {
    ...
    await copyImageToClipboard(blob, "梗图 PNG")
  } finally {
    state.exporting = false
  }
}
...
.text-field>span,.text-presets>span,.style-presets>span,.segment-grid label>span,.option-field>span {
  color: var(--c-text-secondary);
  font-size: 12px;
  font-weight:800
}
.text-presets,.style-presets {
  margin-top:16px
}
.text-presets>div {
  display: flex;
...
@media(max-width:680px) {
  ...
  .style-presets>div,.segment-grid,.slider-grid {
    grid-template-columns:1fr 1fr
  }
  .preview-meta {
    gap:6px
