...
const info = reactive({ title: "横向柱状图" })
...
const canvasHandle = (type) => {
  ...
  switch(type) {
    ...
    case "title":
      ...
      break
    case "color":
      myChart.value?.setOption({ series: [{ itemStyle: { color: attrColor.value } }] })
      break;
    case "watermark":
...
const colunmData = ref(['巴西', '印尼', '美国', '印度', '中国', '世界']);
const valueData = ref(['18203', '23489', '29034', '104970', '131744', '630230']);

const option = {
  backgroundColor: 'transparent',
  title: { text: title.value, subtext: subTitle.value, left: titlePos.value },
  xAxis: {},
  yAxis: { type: 'category', data: colunmData.value },
  series: [{ type: 'bar', data: valueData.value, itemStyle: { color: attrColor.value } }],
  tooltip: {}
};
...
const downEchartsImg = () => {
  ...
  if (imgUrl != undefined) {
    var a = document.createElement('a'); a.href = imgUrl; a.download = 'echart.' + ext; a.click();
  }
}
...
<template>
  <div class="flex flex-col mt-3 flex-1">
    ...
    <ToolDetail title="描述">
      <el-text>
        横向柱状图（条形图）将柱状图的 X/Y 轴互换，以水平条形展示数据，适合展示排行榜、分类对比等场景，尤其当分类标签较长时更易阅读。<br>
        在线制作横向柱状图，支持导入表格并在线编辑，支持 PNG、JPEG 格式导出。<br>
      </el-text>
    </ToolDetail>
  </div>
</template>
