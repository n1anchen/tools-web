...
const canvasHandle = (type) => {
  ...
  switch(type) {
    ...
    case "title":
      let fakerTitle = ''
      let fakerSubTitle = ''
      if (titleSwitch.value === true) fakerTitle = title.value
      if (subTitleSwitch.value === true) fakerSubTitle = subTitle.value
      myChart.value?.setOption({
        title: { text: fakerTitle, subtext: fakerSubTitle, left: titlePos.value }
      })
      break
    case "color":
      myChart.value?.setOption({
        series: [{ itemStyle: { color: attrColor.value }, areaStyle: { color: attrColor.value, opacity: areaOpacity.value / 100 } }]
      })
      break;
    case "areaOpacity":
      myChart.value?.setOption({
        series: [{ areaStyle: { opacity: areaOpacity.value / 100 } }]
      })
      break;
    case "watermark":
      if (watermarkSwitch.value === true) {
        myChart.value?.setOption({ backgroundColor: { image: createWatermark() } })
      } else {
        myChart.value?.setOption({ backgroundColor: 'transparent' })
      }
      break;
    case "data":
      myChart.value?.setOption({
            ...
            <div>
              ...
              <div class="space-y-2 pl-2">
                ...
                <div class="flex items-center gap-2">
                  ...
                  <el-slider v-model="areaOpacity" :min="0" :max="100" :step="5" size="small" class="flex-1" @change="canvasHandle('areaOpacity')" />
                </div>
              </div>
            </div>
            <div class="border-t border-slate-100 dark:border-slate-700"></div>
