    ...
    <ToolHero :summary="meta.headline">
      <template #metrics>
        <div class="hero-metrics">
          <div v-for="metric in heroMetrics" :key="metric.label"><span>{{ metric.label }}</span><strong>{{ metric.value }}</strong></div>
        </div>
      </template>
    </ToolHero>
...
@media(max-width:820px){.chart-family{overflow-x:auto}.chart-family>span,.chart-family a{flex:none}.config-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.palette-section{grid-template-columns:1fr}}
@media(max-width:620px){.chart-page{gap:14px}.hero-metrics{grid-template-columns:1fr}.hero-metrics div{border-left:0;border-bottom:1px solid #e5edf6}.hero-metrics div:last-child{border-bottom:0}.chart-family{border-radius:16px}.sample-card,.data-card,.preview-card,.config-card{padding:15px;border-radius:17px}.sample-card>.section-heading,.card-header{flex-direction:column}.sample-list{grid-template-columns:1fr}.header-actions{width:100%;justify-content:flex-start}.header-actions button{flex:1}.data-card textarea{height:240px}.format-hint{flex-direction:column}.validation-box.success{align-items:flex-start;flex-direction:column}.chart-shell{min-height:340px;height:380px!important}.preview-summary span:first-child{width:100%;margin:0}.config-grid{grid-template-columns:1fr}.palette-list{display:grid;grid-template-columns:1fr 1fr}.palette-list>button,.custom-color{width:100%}.reset-button{width:100%}}
