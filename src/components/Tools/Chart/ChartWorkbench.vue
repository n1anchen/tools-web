...
const typeMeta: Record<ChartKind, {
  ...
  bar: {
    title: '柱状图工作台', eyebrow: 'BAR CHART STUDIO', headline: '把分类差异做得一眼可见',
  ...
  line: {
    title: '折线图工作台', eyebrow: 'LINE CHART STUDIO', headline: '让趋势、拐点与变化速度更清楚',
  ...
  pie: {
    title: '饼图工作台', eyebrow: 'PIE CHART STUDIO', headline: '快速看懂整体由哪些部分构成',
  ...
  scatter: {
    title: '散点图工作台', eyebrow: 'SCATTER CHART STUDIO', headline: '从两个变量中寻找关系与异常点',
  ...
  funnel: {
    title: '漏斗图工作台', eyebrow: 'FUNNEL CHART STUDIO', headline: '把流程流失和关键转化节点摆出来',
...
function downloadPng() {
  ...
  link.download = `${(settings.title || meta.value.title).replace(/[\\/:*?"<>|]/g, '-')}.png`
  link.click()
}
  ...
  <div class="chart-page flex flex-col mt-3 flex-1" :style="{ '--accent': meta.accent, '--accent-soft': meta.accentSoft }">
    ...
    <section class="workspace-grid">
      <article class="data-card">
        ...
        <ChartDataGrid v-if="dataMode === 'grid'" v-model="dataText" :min-columns="gridColumns" :aria-label="`${meta.title}可视数据表格`" @update:model-value="activeSample = ''" />
        <textarea v-else v-model="dataText" spellcheck="false" :aria-label="`${meta.title}数据输入`" @input="activeSample = ''"></textarea>
      ...
      <article class="preview-card">
        ...
        <div class="chart-shell" :style="{ height: `${chartHeight}px` }">
          <div ref="chartElement" class="chart-canvas" role="img" :aria-label="`${meta.title}实时预览，共 ${rows.length} 条数据`"></div>
...
<style scoped>
.chart-page{gap:18px;--accent:#2563eb;--accent-soft:#dbeafe}.hero-card,.chart-family,.sample-card,.data-card,.preview-card,.config-card{border:1px solid #e2e8f0;border-radius:22px;background:#fff;box-shadow:0 14px 38px rgba(15,23,42,.06)}.hero-card{display:grid;grid-template-columns:minmax(0,1fr) auto;align-items:center;gap:28px;padding:30px;border:0;color:#fff;background:radial-gradient(circle at 88% 4%,color-mix(in srgb,var(--accent),#fff 28%),transparent 30%),linear-gradient(135deg,#0f172a,var(--accent) 62%,#0f766e)}.eyebrow{display:block;margin-bottom:6px;color:var(--accent);font-size:12px;font-weight:900;letter-spacing:.15em}.hero-card .eyebrow{color:#fff;opacity:.74}.hero-card h2{margin:0;font-size:clamp(25px,3vw,36px);line-height:1.2}.hero-card p{max-width:730px;margin:11px 0 0;color:rgba(255,255,255,.82);font-size:14px;line-height:1.75}.hero-stats{display:grid;grid-template-columns:repeat(3,116px)}.hero-stats div{padding:8px;text-align:center;border-left:1px solid rgba(255,255,255,.2)}.hero-stats strong,.hero-stats span{display:block}.hero-stats strong{font-size:20px}.hero-stats span{margin-top:5px;color:rgba(255,255,255,.72);font-size:12px}.chart-family{display:flex;min-height:58px;align-items:center;gap:8px;padding:9px 12px;scrollbar-width:none}.chart-family::-webkit-scrollbar{display:none}.chart-family>span{padding:0 10px;color:#64748b;font-size:13px;font-weight:800}.chart-family a{padding:9px 14px;border:1px solid transparent;border-radius:11px;color:#475569;font-size:13px;font-weight:800;text-decoration:none;transition:.2s}.chart-family a:hover{background:#f8fafc}.chart-family a.active{border-color:color-mix(in srgb,var(--accent),#fff 68%);background:var(--accent-soft);color:var(--accent)}.sample-card{display:grid;grid-template-columns:minmax(220px,.48fr) minmax(0,1.52fr);align-items:center;gap:24px;padding:20px 22px}.section-heading{display:flex;align-items:flex-start;justify-content:space-between;gap:16px}.section-heading h3,.card-header h3{margin:0;color:#0f172a;font-size:19px}.section-heading p,.card-header p{margin:4px 0 0;color:#64748b;font-size:13px;line-height:1.55}.sample-card>.section-heading{display:block}.sample-list{display:grid;grid-template-columns:repeat(3,1fr);gap:10px}.sample-list button{min-width:0;padding:13px 14px;border:1px solid #e2e8f0;border-radius:13px;background:#f8fafc;color:#334155;text-align:left;cursor:pointer;transition:.2s}.sample-list button:hover{border-color:var(--accent)}.sample-list button.active{border-color:color-mix(in srgb,var(--accent),#fff 35%);background:var(--accent-soft);box-shadow:0 0 0 2px color-mix(in srgb,var(--accent),transparent 86%)}.sample-list span,.sample-list small{display:block}.sample-list span{font-size:14px;font-weight:850}.sample-list small{margin-top:4px;color:#64748b;font-size:12px}.workspace-grid{display:grid;grid-template-columns:minmax(340px,.74fr) minmax(0,1.26fr);gap:18px}.data-card,.preview-card,.config-card{padding:21px 22px}.card-header{display:flex;align-items:flex-start;justify-content:space-between;gap:14px}.header-actions{display:flex;flex-wrap:wrap;justify-content:flex-end;gap:7px}.header-actions button,.reset-button{display:inline-flex;min-height:36px;align-items:center;justify-content:center;gap:5px;padding:0 11px;border:1px solid #dbe3ef;border-radius:10px;background:#fff;color:#475569;font-size:12px;font-weight:850;cursor:pointer}.header-actions button:hover,.reset-button:hover{border-color:var(--accent);color:var(--accent)}.header-actions button.primary{border-color:var(--accent);background:var(--accent);color:#fff}.mode-tabs{display:grid;grid-template-columns:1fr 1fr;margin-top:17px;padding:4px;border-radius:12px;background:#f1f5f9}.mode-tabs button{min-height:34px;border:0;border-radius:9px;background:transparent;color:#64748b;font-size:13px;font-weight:800;cursor:pointer}.mode-tabs button.active{background:#fff;color:var(--accent);box-shadow:0 2px 8px rgba(15,23,42,.08)}.data-card textarea{display:block;width:100%;height:272px;resize:vertical;margin-top:12px;padding:14px;border:1px solid #dbe3ef;border-radius:13px;outline:0;background:#f8fafc;color:#0f172a;font:700 13px/1.65 ui-monospace,SFMono-Regular,Consolas,monospace;transition:.2s}.data-card textarea:focus{border-color:var(--accent);box-shadow:0 0 0 3px color-mix(in srgb,var(--accent),transparent 88%)}.format-hint{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-top:9px;color:#64748b;font-size:12px;line-height:1.5}.format-hint b{flex:none;color:var(--accent)}.validation-box{margin-top:12px;padding:12px 13px;border-radius:12px;background:#fff7ed;color:#c2410c;font-size:12px;line-height:1.6}.validation-box strong{display:block;font-size:13px}.validation-box ul{margin:5px 0 0;padding-left:18px}.validation-box small{display:block;margin-top:4px}.validation-box.success{display:flex;align-items:center;gap:9px;background:#ecfdf5;color:#047857}.validation-box.success span{color:#64748b}.chart-shell{position:relative;min-height:360px;margin-top:13px;overflow:hidden;border:1px solid #e2e8f0;border-radius:16px;background:linear-gradient(180deg,#fff,#f8fafc);transition:height .2s}.chart-canvas{width:100%;height:100%}.chart-empty{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;color:#94a3b8;font-size:14px;background:rgba(248,250,252,.82)}.preview-summary{display:flex;flex-wrap:wrap;align-items:center;gap:15px;margin-top:12px;color:#64748b;font-size:12px}.preview-summary span:first-child{margin-right:auto;color:#059669;font-weight:800}.preview-summary i{display:inline-block;width:8px;height:8px;margin-right:6px;border-radius:50%;background:#10b981;box-shadow:0 0 0 4px #d1fae5}.config-card>.card-header{padding-bottom:17px;border-bottom:1px solid #e2e8f0}.config-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px;margin-top:18px}.field{display:flex;min-width:0;flex-direction:column;gap:7px}.field>span,.palette-section>div:first-child>span{color:#475569;font-size:13px;font-weight:800}.field input[type="text"],.field select{width:100%;height:40px;padding:0 11px;border:1px solid #dbe3ef;border-radius:10px;outline:0;background:#fff;color:#0f172a;font-size:13px}.field input[type="text"]:focus,.field select:focus{border-color:var(--accent);box-shadow:0 0 0 3px color-mix(in srgb,var(--accent),transparent 88%)}.segmented{display:grid;grid-auto-flow:column;grid-auto-columns:1fr;padding:3px;border:1px solid #dbe3ef;border-radius:10px;background:#f8fafc}.segmented button{min-height:32px;padding:0 9px;border:0;border-radius:7px;background:transparent;color:#64748b;font-size:12px;font-weight:800;cursor:pointer}.segmented button.active{background:#fff;color:var(--accent);box-shadow:0 2px 8px rgba(15,23,42,.08)}.range{width:100%;height:40px;accent-color:var(--accent)}.switch-field{flex-direction:row;align-items:center;justify-content:space-between;padding:0 13px;border:1px solid #dbe3ef;border-radius:10px}.palette-section{display:grid;grid-template-columns:190px 1fr;align-items:center;gap:18px;margin-top:18px;padding-top:18px;border-top:1px solid #e2e8f0}.palette-section>div:first-child small{display:block;margin-top:4px;color:#64748b;font-size:12px;line-height:1.5}.palette-list{display:flex;flex-wrap:wrap;gap:9px}.palette-list>button,.custom-color{display:grid;grid-template-columns:repeat(4,14px);align-items:center;gap:3px;min-height:52px;padding:8px 10px;border:1px solid #dbe3ef;border-radius:11px;background:#fff;color:#475569;cursor:pointer}.palette-list>button.active{border-color:var(--accent);box-shadow:0 0 0 2px color-mix(in srgb,var(--accent),transparent 86%)}.palette-list i{width:14px;height:14px;border-radius:4px}.palette-list span{grid-column:1/-1;font-size:12px;font-weight:800}.custom-color{display:flex;min-width:108px;flex-direction:column;justify-content:center}.custom-color input{width:38px;height:20px;padding:0;border:0;background:transparent;cursor:pointer}.detail-copy{color:#64748b;font-size:14px;line-height:1.9}
:global(html.dark .chart-page .chart-family),:global(html.dark .chart-page .sample-card),:global(html.dark .chart-page .data-card),:global(html.dark .chart-page .preview-card),:global(html.dark .chart-page .config-card){border-color:#334155;background:#1e293b;box-shadow:none}:global(html.dark .chart-page h3){color:#f8fafc}:global(html.dark .chart-page .chart-family a),:global(html.dark .chart-page .sample-list button),:global(html.dark .chart-page .field>span),:global(html.dark .chart-page .palette-section>div:first-child>span){color:#cbd5e1}:global(html.dark .chart-page .chart-family a:hover),:global(html.dark .chart-page .sample-list button),:global(html.dark .chart-page .mode-tabs),:global(html.dark .chart-page .segmented){border-color:#475569;background:#0f172a}:global(html.dark .chart-page .chart-family a.active),:global(html.dark .chart-page .sample-list button.active){border-color:var(--accent);background:color-mix(in srgb,var(--accent),#0f172a 76%);color:#fff}:global(html.dark .chart-page .header-actions button),:global(html.dark .chart-page .reset-button),:global(html.dark .chart-page .palette-list>button),:global(html.dark .chart-page .custom-color),:global(html.dark .chart-page .field input[type="text"]),:global(html.dark .chart-page .field select){border-color:#475569;background:#0f172a;color:#e2e8f0}:global(html.dark .chart-page .header-actions button.primary){border-color:var(--accent);background:var(--accent);color:#fff}:global(html.dark .chart-page .mode-tabs button.active),:global(html.dark .chart-page .segmented button.active){background:#334155;color:#fff}:global(html.dark .chart-page .data-card textarea){border-color:#475569;background:#0f172a;color:#e2e8f0}:global(html.dark .chart-page .chart-shell){border-color:#334155;background:#0f172a}:global(html.dark .chart-page .chart-empty){background:rgba(15,23,42,.86)}:global(html.dark .chart-page .config-card>.card-header),:global(html.dark .chart-page .palette-section){border-color:#334155}:global(html.dark .chart-page .validation-box.success){background:#12372a}:global(html.dark .chart-page .validation-box.success span){color:#94a3b8}
...
@media(max-width:820px){.hero-card{grid-template-columns:1fr}.hero-stats{grid-template-columns:repeat(3,1fr)}.hero-stats div:first-child{border-left:0}.chart-family{overflow-x:auto}.chart-family>span,.chart-family a{flex:none}.config-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.palette-section{grid-template-columns:1fr}}
@media(max-width:620px){.chart-page{gap:14px}.hero-card{padding:22px 18px}.hero-card h2{font-size:25px}.hero-stats div{padding:7px 3px}.hero-stats strong{font-size:17px}.chart-family{border-radius:16px}.sample-card,.data-card,.preview-card,.config-card{padding:15px;border-radius:17px}.sample-card>.section-heading,.card-header{flex-direction:column}.sample-list{grid-template-columns:1fr}.header-actions{width:100%;justify-content:flex-start}.header-actions button{flex:1}.data-card textarea{height:240px}.format-hint{flex-direction:column}.validation-box.success{align-items:flex-start;flex-direction:column}.chart-shell{min-height:340px;height:380px!important}.preview-summary span:first-child{width:100%;margin:0}.config-grid{grid-template-columns:1fr}.palette-list{display:grid;grid-template-columns:1fr 1fr}.palette-list>button,.custom-color{width:100%}.reset-button{width:100%}}
