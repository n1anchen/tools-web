  ...
  <div class="split-logo-page flex flex-col mt-3 flex-1">
    ...
    <section class="preview-card">
      <header class="preview-heading">
        ...
        <div class="export-actions"><el-button :icon="CopyDocument" aria-label="复制双栏徽标 PNG" :disabled="state.loading || state.exporting" @click="copyImage">复制 PNG</el-button><el-button type="primary" :icon="Download" aria-label="下载双栏徽标 PNG" :disabled="state.loading || state.exporting" @click="downloadImage">下载图片</el-button></div>
      </header>
      <div class="canvas-stage">
        ...
        <div v-else-if="state.error" class="loading-overlay error"><span>{{ state.error }}</span><el-button type="danger" @click="drawLogo">重新渲染</el-button></div>
      </div>
    ...
    <div class="workspace-grid">
      <section class="control-card">
        <header class="card-heading"><div><span class="eyebrow">WORDMARK LAYOUT</span><h3>文字与版式</h3></div><el-button :icon="Refresh" aria-label="恢复默认徽标设置" @click="resetStudio">重置</el-button></header>
        ...
        <div class="text-presets"><span>快速文案</span><div><el-button v-for="preset in textPresets" :key="preset.label" size="small" @click="applyText(preset)">{{ preset.label }}</el-button></div></div>
      ...
      <aside class="appearance-card">
        ...
        <div class="theme-presets"><el-button v-for="preset in themePresets" :key="preset.label" @click="applyTheme(preset)"><i :style="{ background: `linear-gradient(135deg, ${preset.background} 50%, ${preset.box} 50%)` }"></i><span><strong>{{ preset.label }}</strong><small>{{ preset.note }}</small></span></el-button></div>
...
.preview-card,.control-card,.appearance-card {
  ...
  box-shadow:0 16px 40px rgba(120,53,15,.07)
}
.eyebrow {
  display: block;
  margin-bottom: 6px;
...
.canvas-stage {
  ...
  background-image: linear-gradient(45deg,#e4e4e7 25%,transparent 25%,transparent 75%,#e4e4e7 75%),linear-gradient(45deg,#e4e4e7 25%,transparent 25%,transparent 75%,#e4e4e7 75%);
...
.canvas-stage canvas {
  ...
  max-width: 100%;
  height: auto;
  filter:drop-shadow(0 13px 25px rgba(24,24,27,.16))
}
.loading-overlay {
  position: absolute;
  inset: 0;
...
.theme-presets :deep(.el-button) {
  width: 100%;
  height: auto;
  padding: 8px;
  white-space: normal;
}
.theme-presets :deep(.el-button > span) {
  display: flex;
  align-items: center;
  gap: 8px;
}
...
.theme-presets span,.theme-presets strong,.theme-presets small {
  display:block
}
.theme-presets strong {
  color: #3f3f46;
  font-size:10px
}
.theme-presets small {
...
.dimension-summary {
  ...
  background:linear-gradient(135deg,#18181b,#78350f)
}
...
:global(html.dark .split-logo-page .preview-card),:global(html.dark .split-logo-page .control-card),:global(html.dark .split-logo-page .appearance-card) {
  border-color: #3f3f46;
  background: #27272a;
  box-shadow:none
}
:global(html.dark .split-logo-page .preview-heading h3),:global(html.dark .split-logo-page .card-heading h3),:global(html.dark .split-logo-page .theme-presets strong) {
  color:#fafafa
}
:global(html.dark .split-logo-page .canvas-stage) {
  background-color: #18181b;
  background-image:linear-gradient(45deg,#27272a 25%,transparent 25%,transparent 75%,#27272a 75%),linear-gradient(45deg,#27272a 25%,transparent 25%,transparent 75%,#27272a 75%)
}
:global(html.dark .split-logo-page .loading-overlay) {
  background:rgba(24,24,27,.82)
}
:global(html.dark .split-logo-page .preview-meta span) {
  color: #d4d4d8;
  background:#18181b
}
:global(html.dark .split-logo-page .color-grid label) {
  border-color:#3f3f46
}
:global(html.dark .split-logo-page .color-grid code) {
  color:#e4e4e7
}
.canvas-stage canvas {
  max-height:520px
...
@media(max-width:650px) {
  ...
  .text-grid,.compact-sliders,.color-grid {
    grid-template-columns:1fr
  }
  .theme-presets {
    grid-template-columns:1fr 1fr
