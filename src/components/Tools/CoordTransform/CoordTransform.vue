...
.single-workspace,.batch-workspace,.precision-note {
...
.map-footer span,.source-note span {
  display: block;
  font-size: 12px;
  color:#7b8ba1
}
...
.single-workspace,.batch-workspace {
  padding:18px
}
.status-strip {
  display: flex;
...
.status-dot {
  ...
  border-radius: 50%;
  background: #35b87f;
  box-shadow:0 0 0 5px rgba(53,184,127,.12)
}
.status-strip strong {
  font-size: 14px;
  color:var(--ink)
...
.map-offline>span,.result-empty>span {
  display: grid;
  place-items: center;
  width: 52px;
  height: 52px;
...
.map-offline strong,.result-empty strong {
  margin-top: 12px;
  font-size: 16px;
  color:var(--ink)
}
.map-offline p,.result-empty p {
  font-size: 13px;
  color:var(--muted)
}
.map-footer {
...
.system-card.active {
  border-color: #80aff7;
  background: #f6faff;
  box-shadow:0 0 0 3px rgba(52,120,246,.06)
}
.system-heading {
  display: flex;
  justify-content: space-between;
...
.system-name strong,.system-name small {
  display:block
}
.system-name strong {
  font-size: 14px;
...
.coordinate-fields label>span,.select-label>span,.batch-editor>span {
  display: block;
  margin-bottom: 5px;
  font-size: 12px;
  font-weight: 600;
...
.result-table td strong,.result-table td span {
  display:block
}
...
.dark .single-workspace,.dark .batch-workspace,.dark .precision-note {
  border-color: #334155;
  background:#172033
}
...
.dark .single-workspace .status-strip {
  border-color: #294e71;
  background:#152c42
}
.dark .map-card,.dark .converter-card,.dark .batch-input-card,.dark .batch-result-card {
  border-color: #334155;
  background:#111b2c
}
.dark .map-stage,.dark .system-card,.dark .result-table-wrap {
  border-color: #334155;
  background:#172033
}
.dark .system-card.active {
  border-color: #467fc7;
  background:#172a41
}
.dark .source-note,.dark .map-footer,.dark .format-hint {
  background:#172a3d
}
.dark .system-name>span {
  background:#26364b
}
...
.dark .result-table td button {
  border-color: #3b608b;
  background: #1a304a;
  color:#9fc7f4
}
.dark .map-offline,.dark .result-empty {
  border-color: #3a495e;
  background:#111a2a
}
.dark .precision-note>span {
  background:#3b2d19
}
.dark .detail-copy {
  color:#aab7c8
...
.system-name>span,.result-table td span {
  font-size:12px
}
</style>
