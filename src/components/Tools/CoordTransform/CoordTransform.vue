...
function buildPopupHtml() {
  ...
  const popupLines = systemMeta.map((system) => {
    const pair = getFormattedPair(system.key, state.current![system.key])
    return `<div style="font-size:12px;line-height:1.6"><span style="color: var(--c-text-secondary)">${system.label}</span> ${pair.primary}, ${pair.secondary}</div>`
  })
...
<template>
  <div class="coord-tool flex flex-col mt-3 flex-1">
    <ToolHero summary="单点定位与批量互转，一个入口完成" description="在五种常用坐标系之间同步换算；单点模式可以地图选点，批量模式适合表格数据和接口清单。">
      <template #metrics>
        <MetricsBar :items="[{ label: '支持坐标系', value: '5 种' }, { label: '当前基准', value: workMode === 'single' ? lastSourceMeta.label : batchSourceMeta.label }, { label: '处理状态', value: workMode === 'single' ? sourcePairText : `${batchResults.length} 个结果` }]" />
      </template>
    </ToolHero>
...
<style scoped>
.coord-tool{--blue:#3478f6;--ink:#263247;--muted:#64748b}.single-workspace,.batch-workspace,.precision-note{border:1px solid #dce6f2;background:var(--c-surface);border-radius:24px;box-shadow:0 12px 32px rgba(51,65,85,.06)}.section-header span{font-size:12px;font-weight:800;letter-spacing:.14em;color:#5277a6}.section-header p{margin:0;font-size:14px;line-height:1.7;color:var(--muted)}.map-footer span,.source-note span{display:block;font-size:12px;color:#7b8ba1}.mode-tabs{display:grid;grid-template-columns:repeat(2,1fr);gap:12px;margin:14px 0}.mode-tabs button{display:flex;align-items:center;gap:12px;padding:14px 18px;text-align:left;border:1px solid #dbe5ef;border-radius:17px;background:var(--c-surface);color:var(--ink);transition:.2s}.mode-tabs button.active{border-color:#7eb0fb;background:#f2f8ff;box-shadow:0 0 0 3px rgba(52,120,246,.08)}.tab-icon{display:grid;place-items:center;width:38px;height:38px;border-radius:var(--radius-md);background:#edf3fa;color:#3971b7;font-weight:800}.mode-tabs strong,.mode-tabs small{display:block}.mode-tabs strong{font-size:15px}.mode-tabs small{margin-top:3px;font-size:12px;color:#7a899d}.single-workspace,.batch-workspace{padding:18px}.status-strip{display:flex;align-items:center;justify-content:space-between;gap:18px;padding:14px 16px;border:1px solid #d9e9ff;border-radius:var(--radius-lg);background:#f4f9ff}.status-strip>div:first-child{display:flex;align-items:flex-start;gap:10px}.status-dot{width:9px;height:9px;margin-top:6px;border-radius:50%;background:#35b87f;box-shadow:0 0 0 5px rgba(53,184,127,.12)}.status-strip strong{font-size:14px;color:var(--ink)}.status-strip p{margin:3px 0 0;font-size:13px;color:var(--muted)}.status-actions,.batch-actions{display:flex;gap:8px;flex-wrap:wrap}.status-actions button,.batch-actions button,.export-button{padding:9px 13px;border:1px solid #cfdbe8;border-radius:var(--radius-sm);background:var(--c-surface);font-size:13px;color:#42516a}.status-actions button:hover,.batch-actions button:hover{border-color:#78a9f4}.status-actions .muted{color:#8794a7}.single-layout,.batch-layout{display:grid;grid-template-columns:minmax(0,1.12fr) minmax(420px,.88fr);gap:16px;margin-top:16px}.map-card,.converter-card,.batch-input-card,.batch-result-card{min-width:0;border:1px solid #e0e8f1;border-radius:var(--radius-card);background:#fbfdff;padding:18px}.section-header{display:flex;justify-content:space-between;gap:14px}.section-header h3{margin:4px 0;font-size:19px;color:var(--ink)}.system-badge{align-self:flex-start;padding:7px 10px;border-radius:var(--radius-full);background:#ebf4ff!important;color:#3972b7!important;letter-spacing:0!important}.quick-locations{display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin:16px 0 10px}.quick-locations span{font-size:13px;color:#718096}.quick-locations button{padding:6px 11px;border:1px solid #d8e2ed;border-radius:var(--radius-full);background:white;font-size:13px;color:#536176}.map-stage{overflow:hidden;border:1px solid #dbe4ee;border-radius:17px;background:#edf2f7}.map-container,.map-offline{height:560px}.map-offline{display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;border:1px dashed #cbd7e5;border-radius:17px;background:#f3f6fa}.map-offline>span,.result-empty>span{display:grid;place-items:center;width:52px;height:52px;border-radius:var(--radius-lg);background:#e8eff7;color:#607b9c;font-weight:800}.map-offline strong,.result-empty strong{margin-top:12px;font-size:16px;color:var(--ink)}.map-offline p,.result-empty p{font-size:13px;color:var(--muted)}.map-footer{display:flex;justify-content:space-between;gap:14px;margin-top:12px;padding:13px;border-radius:var(--radius-md);background:#f0f6fc}.map-footer strong{font-size:16px;color:#2f6fba}.map-footer p{margin:0;max-width:55%;font-size:12px;line-height:1.6;color:#718096}.system-list{display:grid;gap:10px;margin-top:15px}.system-card{padding:13px;border:1px solid #dfe7f0;border-radius:var(--radius-md);background:var(--c-surface);transition:.2s}.system-card.active{border-color:#80aff7;background:#f6faff;box-shadow:0 0 0 3px rgba(52,120,246,.06)}.system-heading{display:flex;justify-content:space-between;gap:10px}.system-name{display:flex;align-items:flex-start;gap:9px;min-width:0;text-align:left}.system-name>span{padding:5px 7px;border-radius:var(--radius-xs);background:#edf3fa;font-size:11px;color:#55708e}.system-name strong,.system-name small{display:block}.system-name strong{font-size:14px;color:var(--ink)}.system-name small{overflow:hidden;margin-top:2px;max-width:280px;font-size:12px;color:#7b899d;text-overflow:ellipsis;white-space:nowrap}.copy-button{padding:5px 9px;border:0;border-radius:var(--radius-xs);background:#edf4ff;font-size:12px;color:#3c72b5}.coordinate-fields{display:grid;grid-template-columns:1fr 1fr;gap:9px;margin-top:10px}.coordinate-fields label>span,.select-label>span,.batch-editor>span{display:block;margin-bottom:5px;font-size:12px;font-weight:600;color:#6c7b8f}.helper-text{margin:7px 0 0;font-size:12px;color:#8a97a8}.error-card,.batch-errors{margin-top:12px;padding:12px;border:1px solid #fed7aa;border-radius:var(--radius-md);background:#fff7ed;font-size:13px;color:#9a5b18}.source-note{margin-top:12px;padding:13px;border-radius:var(--radius-md);background:#eef5fc}.source-note strong{display:block;margin-top:2px;font-size:14px;color:var(--ink)}.source-note p{margin:3px 0 0;font-size:12px;color:var(--muted)}.batch-workspace{min-height:590px}.batch-layout{grid-template-columns:minmax(340px,.7fr) minmax(0,1.3fr);margin-top:0}.select-label,.batch-editor{display:block;margin-top:16px}.format-hint{margin:12px 0;padding:13px;border:1px solid #dbe8f7;border-radius:var(--radius-md);background:#f1f7fd}.format-hint strong{font-size:13px;color:#3e5d7e}.format-hint p{margin:4px 0 0;font-size:12px;line-height:1.6;color:#6c7d91}.batch-actions .primary,.export-button{border-color:#3478f6;background:#3478f6;color: var(--c-on-accent)}.batch-actions button{padding:10px 15px}.result-header{align-items:center}.export-button:disabled,.batch-actions button:disabled{opacity:.45;cursor:not-allowed}.batch-errors strong{display:block}.batch-errors p{margin:4px 0}.batch-errors small{font-size:12px}.result-table-wrap{overflow:auto;max-height:540px;margin-top:14px;border:1px solid #e1e8f0;border-radius:var(--radius-md)}.result-table{width:100%;min-width:820px;border-collapse:collapse;font-size:12px}.result-table th{position:sticky;top:0;z-index:1;padding:11px;background:#edf4fb;color:#63748a;text-align:left}.result-table td{padding:11px;border-top:1px solid #e8edf3;color:#425168;line-height:1.7;font-family:ui-monospace,SFMono-Regular,Menlo,monospace}.result-table td:first-child{font-family:inherit}.result-table td strong,.result-table td span{display:block}.result-table td span{font-size:11px;color:#8b98a9}.result-table td button{padding:6px 9px;border:1px solid #bdd3f1;border-radius:var(--radius-xs);background:#f2f7fd;color:#376da9;white-space:nowrap}.result-empty{display:flex;min-height:410px;flex-direction:column;align-items:center;justify-content:center;text-align:center}.precision-note{display:flex;align-items:flex-start;gap:12px;margin-top:14px;padding:16px 18px}.precision-note>span{display:grid;place-items:center;flex:0 0 38px;height:38px;border-radius:var(--radius-sm);background:#fff1d9;color:#b56a11;font-weight:800}.precision-note strong{font-size:14px;color:var(--ink)}.precision-note p{margin:4px 0 0;font-size:13px;line-height:1.7;color:var(--muted)}.detail-copy{font-size:14px;line-height:1.9;color: var(--c-text-secondary)}.detail-copy p{margin:0 0 8px}.dark .single-workspace,.dark .batch-workspace,.dark .precision-note{border-color:#334155;background:#172033}.dark .section-header p,.dark .status-strip p{color:#a7b4c6}.dark .mode-tabs button,.dark .status-actions button,.dark .quick-locations button{border-color:#38475c;background:#172033;color:#d4deeb}.dark .mode-tabs button.active{border-color:#467fc7;background:#172c44}.dark .single-workspace .status-strip{border-color:#294e71;background:#152c42}.dark .map-card,.dark .converter-card,.dark .batch-input-card,.dark .batch-result-card{border-color:#334155;background:#111b2c}.dark .map-stage,.dark .system-card,.dark .result-table-wrap{border-color:#334155;background:#172033}.dark .system-card.active{border-color:#467fc7;background:#172a41}.dark .source-note,.dark .map-footer,.dark .format-hint{background:#172a3d}.dark .system-name>span{background:#26364b}.dark .result-table th{background:#213047;color:#a9b6c7}.dark .result-table td{border-color:#2c3b50;color:#c4cfdd}.dark .result-table td button{border-color:#3b608b;background:#1a304a;color:#9fc7f4}.dark .map-offline,.dark .result-empty{border-color:#3a495e;background:#111a2a}.dark .precision-note>span{background:#3b2d19}.dark .detail-copy{color:#aab7c8}@media(max-width:1100px){.single-layout,.batch-layout{grid-template-columns:1fr}.map-container,.map-offline{height:430px}}@media(max-width:640px){.mode-tabs{grid-template-columns:1fr}.single-workspace,.batch-workspace{padding:12px}.status-strip,.map-footer{align-items:flex-start;flex-direction:column}.status-actions{width:100%}.status-actions button{flex:1}.map-card,.converter-card,.batch-input-card,.batch-result-card{padding:14px}.coordinate-fields{grid-template-columns:1fr}.map-container,.map-offline{height:330px}.system-name small{max-width:190px}.map-footer p{max-width:none}.result-header{align-items:flex-start;flex-direction:column}.export-button{width:100%}.precision-note{border-radius:var(--radius-lg)}}
.system-name>span,.result-table td span{font-size:12px}</style>
