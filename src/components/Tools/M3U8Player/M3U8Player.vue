...
.player-heading > div:last-child > span {
  color: var(--c-text-muted);
...
.status-indicator.loading i, .status-indicator.buffering i {
  background: #f59e0b;
  animation: blink 1s ease infinite;
}
.status-indicator.ready i, .status-indicator.playing i {
  background: #22c55e;
...
.video-placeholder, .loading-overlay, .error-overlay {
  position: absolute;
  display: flex;
  inset: 0;
  align-items: center;
  ...
  flex-direction: column;
  padding: 25px;
  background: radial-gradient(circle at center, #172554, #030712 68%);
  text-align: center;
}
...
.video-placeholder span, .loading-overlay span, .error-overlay span {
  max-width: 80%;
  margin-top: 4px;
  overflow: hidden;
  color: var(--c-text-muted);
...
.error-overlay {
  background: radial-gradient(circle at center, #450a0a, #030712 70%);
}
.error-overlay span {
  margin-bottom: 14px;
  color: #fca5a5;
...
.player-actions label > span {
  color: var(--c-text-muted);
  font-size: 12px;
}
.player-actions :deep(.el-select) {
  width: 190px;
}
.diagnostics-card {
  position: sticky;
...
:global(html.dark .hls-page .source-card), :global(html.dark .hls-page .player-card), :global(html.dark .hls-page .diagnostics-card), :global(html.dark .hls-page .recent-card), :global(html.dark .hls-page .tips-grid article) {
  border-color: var(--c-border);
  background: var(--c-surface);
  box-shadow: none;
}
:global(html.dark .hls-page h3), :global(html.dark .hls-page .stream-stats strong), :global(html.dark .hls-page .subheading strong), :global(html.dark .hls-page .tips-grid strong) {
  color: #f8fafc;
}
...
:global(html.dark .hls-page .stream-stats div), :global(html.dark .hls-page .player-actions) {
  border-color: var(--c-border);
}
:global(html.dark .hls-page .log-list article) {
  background: #422006;
}
:global(html.dark .hls-page .log-list article.fatal) {
  background: #450a0a;
}
:global(html.dark .hls-page .log-list code) {
  color: #fed7aa;
}
...
@media (max-width: 640px) {
  ...
  .source-card, .diagnostics-card, .recent-card {
    border-radius: 19px;
  }
  .player-heading, .player-actions {
    align-items: flex-start;
    flex-direction: column;
  }
  .player-heading > div:last-child, .player-actions > div, .player-actions label, .player-actions :deep(.el-select) {
    width: 100%;
  }
  .player-actions > div {
    display: grid;
