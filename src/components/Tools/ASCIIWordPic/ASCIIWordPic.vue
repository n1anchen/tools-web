...
<template>
  <div class="ascii-page flex flex-col mt-3 flex-1">
    <ToolHero summary="把普通文字，变成终端里的标题">
      <template #metrics>
        <MetricsBar :items="[{ label: '本地字体', value: '21' }, { label: '输出行数', value: metrics.rows || '—' }, { label: '最大列宽', value: metrics.columns || '—' }]" />
      </template>
    </ToolHero>
...
<style scoped>
.ascii-page{--accent:#8b5cf6;--accent-deep:#6d28d9;--soft:#f5f3ff;gap:16px}.control-card,.preview-card{border: 1px solid var(--c-border);border-radius:var(--radius-card);background:var(--c-surface);box-shadow: var(--shadow-card)}.eyebrow{display:block;margin-bottom:6px;color:var(--accent);font-size:10px;font-weight:900;letter-spacing:.16em}.studio-grid{display:grid;grid-template-columns:minmax(330px,.72fr) minmax(0,1.28fr);gap:16px;align-items:stretch}.control-card,.preview-card{padding:21px}.card-heading,.preview-heading{display:flex;align-items:center;justify-content:space-between;gap:12px}.card-heading{margin-bottom:18px}.card-heading h3{margin:0;color: var(--c-text-primary);font-size:19px}.live-status{padding:5px 8px;border-radius:var(--radius-full);color:#047857;background:#d1fae5;font-size:9px;font-weight:800}.live-status.busy{color:#92400e;background:#fef3c7}.live-status.error{color:#be123c;background:#ffe4e6}.field-label,.slider-setting label{display:flex;align-items:center;justify-content:space-between;margin-bottom:7px;color: var(--c-text-secondary);font-size:11px}.field-label b,.slider-setting strong{color:var(--accent-deep)}.input-warning{margin:7px 0 0;color:#b45309;font-size:10px}.sample-row,.recent-row{display:flex;align-items:center;flex-wrap:wrap;gap:6px;margin-top:12px}.sample-row>span,.recent-row>span{margin-right:4px;color: var(--c-text-muted);font-size:10px}.sample-row button,.recent-row button{padding:5px 8px;border: 1px solid var(--c-border);border-radius:var(--radius-full);color: var(--c-text-secondary);background: var(--c-surface-subtle);cursor:pointer;font-size:9px}.setting-grid{display:grid;grid-template-columns:1fr 1.2fr;gap:12px;margin-top:18px}.setting-grid label>span{display:block;margin-bottom:7px;color: var(--c-text-secondary);font-size:10px}.setting-grid :deep(.el-segmented){width:100%}.slider-setting{margin-top:14px}.slider-setting :deep(.el-slider){padding:0 8px}.auto-row{display:flex;align-items:center;justify-content:space-between;margin-top:10px;padding:11px 12px;border-radius:var(--radius-md);background: var(--c-surface-subtle)}.auto-row strong,.auto-row span{display:block}.auto-row strong{color: var(--c-text-strong);font-size:11px}.auto-row span{margin-top:2px;color: var(--c-text-muted);font-size:9px}.primary-actions{display:flex;flex-wrap:wrap;gap:7px;margin-top:16px}.primary-actions :deep(.el-button){margin-left:0}.preview-card{min-width:0;padding:0;overflow:hidden;background:#0b1020}.preview-heading{padding:12px 15px;border-bottom:1px solid #252d41;color: var(--c-text-muted);background:#111827}.preview-heading>div{display:flex;align-items:center;gap:8px}.preview-heading strong{font:10px ui-monospace,SFMono-Regular,monospace}.preview-heading button{display:flex;align-items:center;gap:4px;padding:5px 7px;border:1px solid #334155;border-radius:var(--radius-xs);color:#cbd5e1;background:#1e293b;cursor:pointer;font-size:9px}.preview-heading button:disabled{opacity:.35;cursor:not-allowed}.window-dots{display:flex;gap:4px}.window-dots i{width:7px;height:7px;border-radius:50%;background:#fb7185}.window-dots i:nth-child(2){background:#fbbf24}.window-dots i:nth-child(3){background:#34d399}.terminal{min-height:360px;max-height:520px;padding:24px;overflow:auto;color:#d8b4fe;background:radial-gradient(circle at 100% 0,rgba(139,92,246,.16),transparent 32%),#070b16}.terminal pre{min-width:max-content;margin:0;font-family:"SFMono-Regular",Consolas,"Liberation Mono",monospace;line-height:1.16;text-shadow:0 0 18px rgba(192,132,252,.28)}.terminal.empty{display:grid;place-items:center}.preview-state{display:flex;align-items:center;gap:8px;color: var(--c-text-secondary);font-size:11px}.preview-state.error{color:#fda4af}.metric-strip{display:grid;grid-template-columns:repeat(4,1fr);border-top:1px solid #252d41;background:#111827}.metric-strip div{padding:12px;border-right:1px solid #252d41}.metric-strip div:last-child{border-right:0}.metric-strip span,.metric-strip strong{display:block}.metric-strip span{color: var(--c-text-secondary);font-size:8px}.metric-strip strong{margin-top:3px;color:#e2e8f0;font-size:11px}:global(html.dark .ascii-page .control-card){border-color: var(--c-border);background:var(--c-surface);box-shadow:none}:global(html.dark .ascii-page .card-heading h3),:global(html.dark .ascii-page .auto-row strong){color:#f8fafc}:global(html.dark .ascii-page .auto-row),:global(html.dark .ascii-page .sample-row button),:global(html.dark .ascii-page .recent-row button){border-color: var(--c-border);color: var(--c-text-secondary);background:var(--c-surface-subtle)}@media(max-width:1000px){.studio-grid{grid-template-columns:1fr}.terminal{min-height:330px}}@media(max-width:650px){.ascii-page{gap:12px}.control-card{padding:16px}.setting-grid{grid-template-columns:1fr}.preview-heading{align-items:flex-start}.terminal{min-height:280px;padding:16px}.metric-strip{grid-template-columns:1fr 1fr}.metric-strip div:nth-child(2){border-right:0}.metric-strip div:nth-child(-n+2){border-bottom:1px solid #252d41}.primary-actions :deep(.el-button:first-child){flex:1}}</style>
